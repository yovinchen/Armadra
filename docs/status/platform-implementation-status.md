...
| A03 | ~~Claude 状态栏精确用量；Codex/Gemini 结构化转录估算（source=structured_transcript, quality=estimated, 模型容量表）；阈值设置~~（单会话上下文占用整条于 2026-09-21 移除，见 [TS core 状态 §29](typescript-core-status.md)）；CLI `--version` 探测缓存；能力=适配器∩配置∩探测∩执行主机并显示来源；模型选择菜单（新会话生效）                                                                                                                                        | —                        | Copilot（`events.jsonl` 无逐轮 token 字段）与 OpenCode（插件 API 无接口）不声明 contextUsage；Pi/OMP 已精确上报；cron / launchd 规则经 `nativeRecurrence` 预填向导（内置七种 CLI 仍不声明）；压缩后估算为上界 |
