...
| B01 | Runtime `browser` 模块：本机 Chromium 发现、独立 profile、CDP 会话、screencast 帧流与输入注入、Console/Network 摘要、下载队列、Runtime 重启恢复；`armadra-hook browser` 动词只对连线节点开放；`browser.proto`；macOS 真实 Chrome 17 项测试                                                                                               | —                        | 受管浏览器下载、多标签/iframe、控制租约、跨端画面、Windows/Linux                                                 |
| H01 | `canvas.proto` 与 Host canvas 存储/表面（revision CAS、幂等收据、事件 outbox 单调 sequence、快照）；`write_ownership` 记录与 CLI `ownership status/switch/rollback`（先导出→staging 导入→逐项核验→经 Worker 下发 epoch）；Runtime 收到 epoch 后拒绝画布写入（`ownership_moved`）；Web 按探测的写入方路由并按 sequence 续订；端到端 44 项 | —                        | session/agent/filesystem/git 业务表面；HTTPS 切换；反向导入；Host→Web 事件仍为 3 秒轮询                          |
...
| S03 | `updates.proto`、Host `CheckForUpdate`（来源可配、mock 验证）、桌面 `tauri-plugin-updater` 接入与「更新」设置分区、无公钥时明确未配置；服务器模式 `install/uninstall/status/logs/upgrade` 只生成定义文件                                                                                                                                 | —                        | 下载/安装、签名发布、真正注册系统服务                                                                            |
