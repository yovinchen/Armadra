...
> 上面几节量的是帧与重渲，这一节量的是**常驻**：一块空画布摆在那里，壳要占
> 多少。脚本与前几节不是同一个——这里跑的是**装好的 `.app`**，不是开发服务器。

### 6.1 怎么量的

- 装好的 arm64 包，数据目录与 Electron profile 都隔离到 `/tmp`，每一轮都是
  全新的数据目录（`ARMADRA_DATA_DIR` + `--user-data-dir`）。
- 各进程 RSS 用 `ps -o rss=`；渲染进程的 JS 堆 / DOM 节点 / 监听器用 CDP 的
  `Runtime.getHeapUsage` 与 `Memory.getDOMCounters`，采样前先
  `HeapProfiler.collectGarbage`。
- **启动后等 180 s 再采**。前 120 s 里 core 在建会话索引（一次扫 ~2000 条
  对话，RSS 冲到 800 MB），系统有内存压力时其余进程的 RSS 会被压下去——60 s
  采到的数字比 180 s 低 30 MB，那不是壳更省，是页被换出去了。
- 每种构建跑 3 轮取中位数。两种构建在**同一台机器上前后脚**跑完，中间没有
  别的变量。
...
| 指标               | 改动前  | 改动后  | 差       |
| ------------------ | ------- | ------- | -------- |
| 渲染进程 RSS       | 200 MB  | 176 MB  | **−24**  |
| 主进程 RSS         | 168 MB  | 168 MB  | 0        |
| GPU 进程 RSS       | 103 MB  | 103 MB  | 0        |
| 网络 utility RSS   | 51 MB   | 52 MB   | +1       |
| 渲染进程 JS 堆     | 24.3 MB | 21.3 MB | **−3.0** |
| 启动就加载的 JS    | 6.17 MB | 3.02 MB | **−51%** |
| 启动就加载的 chunk | 34 个   | 32 个   | −2       |
| DOM 节点 / 监听器  | 343/385 | 348/380 | 持平     |

一块空画布上没有任何节点、没有终端、没有浏览器，改动前却已经把
`codemirror`（862 kB）、`conflict`（834 kB）、`markdown`（544 kB）、
`language`（153 kB）、`xterm`（443 kB）全部取回并编译了。三笔改动各自的根因
写在提交信息里，合起来是同一件事：**分包分了，但没有人在「什么时候取」上把
门**。

### 6.3 开 5 个终端 + 1 个浏览器，再全部关掉（改动后）

| 时点            | main | renderer | gpu | JS 堆   | DOM | 监听器 |
| --------------- | ---- | -------- | --- | ------- | --- | ------ |
| 空画布空闲 60 s | 173  | 174      | 101 | 21.1 MB | 348 | 376    |
| +5 终端节点     | 196  | 207      | 108 | 26.6 MB | 901 | 845    |
| +1 浏览器节点   | 203  | 371      | 115 | 26.7 MB | 962 | 873    |
| 全部关掉 + 60 s | 198  | 211      | 115 | 24.9 MB | 422 | 530    |

浏览器节点那 160 MB 是 webview 自己的，关掉就退了（371 → 211）。**JS 堆、
DOM 节点与监听器都回落**（26.7 → 24.9 MB、962 → 422、873 → 530），比初始值
各高一点点，是那一轮里被打开过的面板 chunk 留下的——开过一次就一直挂着是
闸门有意的行为（`app/overlay-gates.ts`）。进程 RSS 不回落是 V8 与 Blink 不把
页还给系统，不是泄漏。

### 6.4 主进程为什么没动

`electron-updater` 确实不再在启动路径上了——主进程的 inspector 里
`process.moduleLoadList` 找不到它，`require.cache` 里是 0 条。但 RSS 一点没
变，因为：

```
主进程 heapUsed = 6.1 MB，rss = 161 MB
```

**壳自己的 JavaScript 只占这个进程的 4%。** 剩下的是 Electron 浏览器进程本身
——V8 快照、Blink、网络栈、Skia、macOS 的那几个框架。裸 Node 里量到
`require("electron-updater")` 要 16.5 MB RSS，但在一个已经把 V8 跑热、把那些
页都映射好的浏览器进程里，这 16 MB 大部分是重叠的，落到 RSS 上看不出来。

所以主进程 ≤ 110 MB 这个目标**这条路上到不了**：把壳的 JS 全删光也只省得下
6 MB。要再往下只剩两条，都不在本批范围：关掉用不上的 Chromium 特性
（`--disable-features`，会改行为），或者不再让主进程当浏览器进程（不可能）。

### 6.5 剩下的大头（本批没动）

| 来源                      | 量      | 说明                                                  |
| ------------------------- | ------- | ----------------------------------------------------- |
| zod                       | 582 kB  | 被 API 客户端在启动路径上用，每条响应都要校验，拆不开 |
| `index` 入口              | 689 kB  | 应用自己的主链路                                      |
| `react-vendor` / `xyflow` | 868 kB  | React 与画布本体，空画布也要                          |
| `radix`                   | 386 kB  | 壳与侧栏用的组件                                      |
| core 进程                 | 100–900 | 建会话索引时冲到 800 MB；不在本批范围                 |
