...
> 2026-09-26 按源码复核了下文的断裂 / 回退项与 §5.1 的单向通道，已修复的见 §0；正文保留 09-19 的原始结论，只在对应行标「已修复」。

## 0. 2026-09-26 复核：已修复的项

| 原条目                                                                      | 现状                 | 依据                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| §2.1 人自己点的下载被拦进暂存目录（§7 #1）                                  | 已修复               | `apps/desktop/src/main/browser/transfers.ts` 按发起者分流：人发起的直接存进系统下载目录、重名编号（`shell-core/browser/downloads.ts` 的 `userDownloadName` / `uniqueDownloadPath`）；Agent 引起的仍暂存，启动时清掉一天以上的（`sweepStaging`）。提交 `e7ce038b`                                                                                                         |
| §2.1 设置 → 数据「显示数据目录」按钮必失败（§7 #2）                         | 已修复               | `apps/web/src/panels/settings/pages/DataPage.tsx:78` 改调 `revealPath`，壳新增 `shell:show-item-in-folder`（`apps/desktop/src/shared/ipc.ts:98`），失败有提示而不是只写控制台。提交 `8c73a054`                                                                                                                                                                           |
| §2.1 手机上能打开 / 新建永远不可用的浏览器节点（§7 #3）                     | 已修复               | 入口改看能力：`apps/web/src/shell/mobile-focus.ts:26` 只在 core 报 `headlessBrowser` 时让浏览器节点可聚焦，`canvas/menus/add-menu.ts:158` 的新建项要求桌面壳或 `headlessBrowser`，`nodes/NodeShell.tsx` 与 `shell/MobileFocusPage.tsx:41-49` 同一判定。服务器壳带 headless Chromium 时节点体是画面流（[TS core 进度](typescript-core-status.md) §39.1）。提交 `fabcf0e8` |
| §5.2 / §7 #4 隐藏回收开关硬编码为开                                         | 已修复               | 设置 → 浏览器有回收开关（`apps/web/src/panels/settings/pages/BrowserPage.tsx:72-78`），`nodes/browser/WebviewGuest.tsx:111` 读偏好里的 `discardMinutes`。提交 `29e21640`                                                                                                                                                                                                 |
| §5.1 `window:key-intent` 单向（§2.2 ⌘W「先关节点」、§7 #13）                | 已修复               | 页面在 `apps/web/src/keybindings/window-intents.ts:60-80` 订阅，`close-window` 先关聚焦的节点，并经新增的 `window:key-intent-result`（`ipc.ts:144`）回答接没接住，没接住壳才关窗。提交 `8415cfb5`                                                                                                                                                                        |
| §5.1 `window:notification-click` 单向（§7 #13）                             | 已修复               | 同一文件订阅 `onNotificationClick`，按 `nodeId` 聚焦节点（`window-intents.ts:74-76`）                                                                                                                                                                                                                                                                                    |
| §5.1 `window:is-focused` 无调用者                                           | 已修复（有了调用者） | `apps/web/src/platform/index.ts:195-203` 发系统通知前先问壳窗口是否在前台——无边框窗口里 webview 抢焦点时 `document.hasFocus()` 不可靠                                                                                                                                                                                                                                    |
| §3.2 / §3.3 `pickFiles` 孤儿（§7 #14）                                      | 页面侧已清           | `apps/web/src/platform/index.ts` 不再有 `pickFiles`（提交 `51d7164a`，同批删了 `isDesktopShell` 与 Agent partition）；壳的 `dialog:pick-files`（`ipc.ts:79`、`main/index.ts:120`）仍在，页面没有调用者                                                                                                                                                                   |
| `BUNDLED_WORKSPACE_PACKAGES` 仍写 `@armadra/protocol`（TS core 进度 §18.6） | 已修复               | `apps/desktop/electron.vite.config.ts:32` 只剩 `@armadra/shared`。提交 `144c546a`                                                                                                                                                                                                                                                                                        |
| §2.2 平台自身组件缺 Browser（§2.2 末行）                                    | 已修复               | 壳每 5 秒报 `app.getAppMetrics()`，资源面板多出壳主进程 / 渲染 / GPU / utility 与 `browserGuest` 五种；headless 后端的 Chromium 按树计入（TS core 进度 §39.4）                                                                                                                                                                                                           |
| §3.2 `ActivityLine` 从不渲染、Agent 驱动时的对话框人看不到（§7 #5、#8）     | 已修复               | `apps/web/src/nodes/browser/Lease.tsx:120` 渲染 `ActivityLine`；对话框与文件请求显示在节点头部（提交 `2179d9b8`）                                                                                                                                                                                                                                                        |
| §3.2 `handControl` 失败静默（§7 #6）                                        | 已修复               | `WebviewSurface.tsx` 的接管有 `catch`，失败提示 `browser.lease.failed`                                                                                                                                                                                                                                                                                                   |
| §3.1 i18n 死键（§7 #7）                                                     | 已修复               | `apps/web/src/i18n/i18n.test.ts:199` 起有 `UNREFERENCED` 白名单扫描，新增未引用键会失败                                                                                                                                                                                                                                                                                  |

仍未修复：§2.1 的 Console / Network 摘要（`Network.*` 仍被 `main/browser/sole-call-site.test.ts` 禁止）；§2.1 的「默认不占固定端口」按现状改写为「壳内由内核分配回环端口」，不再当作目标（[功能总表](feature-roadmap.md) §3.12）；iframe 兼容预览不恢复，纯网页端改由 headless 画面流承担。
...
| 3.6  | 导航、尺寸、截图、**Console/Network 摘要**、**下载队列**、登录持久化 ✅ | 断裂（下载已修复，§0；摘要仍缺） | Console/Network 摘要随 `session/console.rs` 删除后未重建，`Network.*` 被结构性测试主动禁止（`apps/desktop/src/main/browser/sole-call-site.test.ts:92-99`）；下载被 `apps/desktop/src/main/browser/transfers.ts:60-99` 全部拦进暂存目录，而唯一出口是 Agent 的 `download --accept`（`main/browser/verbs.ts:515-522`），**人自己点的下载没有任何界面能接受它**，且暂存目录不过期（`transfers.ts:26-28`） |
| 3.11 | 手机焦点页、底部导航、软键盘工具条 🔶                                   | 断裂（已修复，§0）               | 浏览器节点仍在手机可聚焦集合里（`apps/web/src/shell/mobile-focus.ts:13`），入口仍出现在节点菜单（`apps/web/src/nodes/NodeShell.tsx:440-449`）与焦点页切换器（`apps/web/src/shell/MobileFocusPage.tsx:41-44,89-94`），点进去只有一句「只在桌面应用里可用」。新建入口也无门禁（`apps/web/src/canvas/menus/add-menu.ts:238-247`）                                                                         |
...
| 3.13 | 通用 / 数据页 ✅（数据目录「显示」按钮）                                | 断裂（已修复，§0）               | `apps/web/src/panels/settings/pages/DataPage.tsx:78` 调 `openExternal("file://…")`，而壳的白名单只允许 `http`/`https`（`apps/desktop/src/shell-core/external-url.ts:20`），必被 `scheme_not_allowed` 拒；`apps/web/src/platform/index.ts:82-86` 只 `console.error`，界面无任何反馈                                                                                                                     |
...
| 3.3  | Command W 隐藏到托盘；Command Q 停止 Host/Runtime/受管会话 ✅ | 回退（⌘W 先关节点已修复，§0） | ⌘W 与 ⌘Q 两半都在（`apps/desktop/src/shell-core/keydown-intercept.ts:52-59`、`apps/desktop/src/main/lifecycle.ts:70-80`）；「受管会话」一项随受管 Chromium 消失。另：`window:key-intent` 事件主进程已发（`apps/desktop/src/main/menu.ts:46`），但页面**从不订阅**，「⌘W 先关一个节点」这半从未生效    |
| 3.10 | 平台自身组件占用：Session Host、Browser 未有 🔶               | 回退（已修复，§0）            | 浏览器现在是壳自己的 guest 进程，不再是 Runtime 拉起的子进程，Runtime 的进程树采样更没有触达路径；该行的缺口比迁移前更大                                                                                                                                                                              |
...
| 通道                        | 主进程发送点                                      | 页面订阅                   | 后果                                 |
| --------------------------- | ------------------------------------------------- | -------------------------- | ------------------------------------ |
| `window:key-intent`         | `apps/desktop/src/main/menu.ts:46`                | 无（已修复，§0）           | ⌘W「先关一个节点」这半从未生效       |
| `window:notification-click` | `apps/desktop/src/main/notifications.ts:42`       | 无（已修复，§0）           | 主进程通知点击后跳节点无人处理       |
| `window:is-focused`         | `apps/desktop/src/main/index.ts:95`（handler 在） | 无调用者（已有调用者，§0） | 页面改用 `document.hidden`，通道多余 |
...
| 项                                                    | 状态                             | 依据                                                                                                                                                                                                                                                                        |
| ----------------------------------------------------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 设置页的隐藏回收开关                                  | 未接通（已修复，§0）             | `apps/web/src/nodes/browser/WebviewGuest.tsx:298-300` 硬编码 `enabled: true`，注释自陈「接上设置面板时改的是这一行」；设置页无 Browser 页，`packages/shared/src/api/settings.ts` 无相关键；`discard.ts:17-18` 的「定时器触发时重读」形同虚设                                |
...
| #   | 现象                                                                                        | 位置                                                                        | 建议做法                                                                                       | 规模 |
| --- | ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---- |
| 1   | 用户自己点的下载被拦进暂存目录，没有界面能接受（已修复，§0）                                | `apps/desktop/src/main/browser/transfers.ts:60-99`                          | 人发起的下载直接放行到系统下载目录；Agent 发起的才进暂存并加过期                               | M    |
| 2   | 设置→数据「显示数据目录」按钮必失败且无反馈（已修复，§0）                                   | `apps/web/src/panels/settings/pages/DataPage.tsx:78`                        | 新增一条 `shell:show-item-in-folder`（`shell.showItemInFolder`），不放宽 `openExternal` 白名单 | S    |
| 3   | 手机上能打开 / 新建永远不可用的浏览器节点（已修复，§0）                                     | `apps/web/src/shell/mobile-focus.ts:13`、`canvas/menus/add-menu.ts:238-247` | 从可聚焦集合移除 `browser`，新建入口加 `isDesktop()` 门禁                                      | S    |
| 4   | 隐藏回收开关硬编码为开，设置页没有 Browser 页（已修复，§0）                                 | `apps/web/src/nodes/browser/WebviewGuest.tsx:298-300`                       | 加一页设置（回收开关 + 分钟数 + 后台上限），接到 `discard.ts` 已留好的读取点                   | M    |
| 5   | Agent 驱动期间页面弹对话框 / 选文件，人看不到提示（已修复，§0）                             | `apps/runtime/src/browser/shell/mod.rs:166-207` 无页面消费者                | 在租约徽标旁显示一行，或恢复一个精简的接管卡片                                                 | M    |
| 6   | 租约接管 / 标签操作失败静默（已修复，§0）                                                   | `apps/web/src/nodes/browser/WebviewSurface.tsx:146-153`                     | 加 catch，用已有的 `browser.lease.failed` / `browser.tabs.failed`                              | S    |
| 7   | 47 个 i18n 死键（已修复，§0）                                                               | `apps/web/src/i18n/browser.ts`（见 §3.1）                                   | 删键；顺带给 `i18n.test.ts` 加一条「键必须被引用」的扫描守卫                                   | S    |
| 8   | `ActivityLine` 从不渲染，Agent 操作对人不可见（已修复，§0）                                 | `apps/web/src/nodes/browser/Lease.tsx:86-114`                               | 接进 `WebviewSurface` 的头部，或连同两个键一起删                                               | S    |
| 9   | 文档仍写 Tauri / 固定端口 / iframe 回退（功能总表已按现状重写，2026-09-26）                 | 见 §4.1 十八处                                                              | 按清单逐条改；`remote-and-browser-completion.md:4` 的取代范围扩到 §4.1 与 §5                   | S    |
...
| 13  | `window:key-intent` / `window:notification-click` 单向接通（已修复，§0）                    | `apps/desktop/src/main/{menu,notifications}.ts`                             | 页面订阅并实现「先关节点」「跳到节点」，或从 IPC 表删掉两条                                    | S    |
| 14  | 孤儿代码：`pickFiles`、`isDesktopShell`、Agent partition、`Viewport` 常量（页面侧已清，§0） | §3.2                                                                        | 一并清理；`isDesktopShell` 合并进 `isDesktop`                                                  | S    |
| 15  | `cargo test -p armadra-runtime` 按 AGENTS.md 写法必失败（已作废：Rust 已删除）              | `apps/runtime/tests/hook_endpoint_failover.rs:120,132`                      | 改 AGENTS.md 的命令，或让该测试自行 `cargo build -p armadra-hook`                              | S    |
