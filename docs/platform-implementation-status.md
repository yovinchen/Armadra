...
- Runtime新增0003迁移，将旧看板JSON和节点标签/备注原文复制到独立不可变归档；保留原workspace/canvas/node ID、名称、时间，没有级联删除。旧列保留原字节并冻结，新的画布只能带惰性空默认值。
- 当前Board/Save DTO、shared、canvas-store和保存队列不再持有可写Kanban，不再因删除节点裁剪历史卡片。旧kanban写字段（包括null）明确400拒绝；其他既有兼容字段继续处理。绘图、备注和普通节点标签继续工作。
- 设置→数据提供只读元数据分页、原文/摘要与JSON包导出；已删除画布仍可审计，未恢复交互看板。此接口属于当前全局本机Data管理域，后续Host暴露须单独数据管理授权。
- db29、旧写请求拒绝1、export12、Web保存链路/归档115、shared69及类型检查/Clippy通过；Go迁移v3及race通过。实际CLI脚本分别验证v1/v2/v3 Rust导出→Go staging导入，源库字节未改、SQL1/2及Go副本原字节保留。
- 可重复链路为scripts/legacy-archive-smoke.mjs。本批没有触发用户正常数据库迁移或关闭其后台；新版本首次正常启动时才按已验证迁移执行。

## 连续实施：Go→Rust 只读 Worker 桥接

- Runtime新增独立 `worker --stdio` 模式，不启动旧HTTP、迁移数据库或恢复终端。父进程持有匿名管道，Protobuf大端长度帧有1MiB上限；握手绑定Host/instance，后续逐请求验证身份、截止时间与结果oneof。
- 支持规范目录注册、目录读取及SHA-256绑定的UTF-8字节分块；后续分块必须匹配首块版本，文件变化返回冲突。文本暂限1MiB，块限256KiB，目录受500项/帧预算限制；portable路径不能表示的名称明确截断跳过。此批没有开放文件写入、命令执行或PTY。
- Go客户端仅启动显式绝对原生程序，固定argv，不走shell；建立连接的ctx不控制整个子进程寿命，请求串行并验证结果后释放队列。取消/畸形连接关闭并回收自有child，退出不确认则报错；stderr有界且只对外提供摘要。
