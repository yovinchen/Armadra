...
- **注入尽量只在启动那一次。** 能用启动参数或会话级配置的 CLI（Claude 的 `--settings`，Copilot、OpenCode 的会话配置）不再改用户的全局配置，Hook 在拉起会话时注入、会话结束即消失；做不到的（Codex `hooks.json`）继续写文件，但写法幂等、带 Armadra 标记、可修复。技能目录本来就是按 CLI 的固定位置，保持文件安装（技能是模型读的说明书，没有启动参数可注入）。
