...
| 位置                     | 展示形式                                          | 落点                                                                                                                                                                                                      |
| ------------------------ | ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 节点头                   | Agent chip 之后一个 `@codex-1` 小徽标，点击即改名 | `apps/web/src/nodes/NodeShell.tsx` 头部插槽                                                                                                                                                               |
| 终端内状态行（六种全部） | 不做                                              | 我们自己那条 `statusLine` 已随单会话上下文占用一起移除，用户自己的状态行从来不动；名字改由 §2.4 的注入与节点头承担。**不发明**一种往 PTY 里写装饰行的机制——那会污染转录，也会被 `context terminal` 读回去 |
...
| 项   | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文件 | `0021_agent_names.sql`；`core/collab/addressing.ts`（`loadHandles` 改读表）；`core/collab/control/edits.ts`（`rename` 写表、`link --name-*`）；`core/collab/context-link.ts:175`、`core/collab/mailbox.ts` 的 inbox 行、`core/collab/control/nodes.ts` 的 list 行；`core/terminal/environment.ts`（`ARMADRA_NODE_NAME`）；`core/collab/skill.ts` + `SKILLS_REVISION`；`apps/web`：`canvas/connection.ts` 命名弹窗、`nodes/NodeShell.tsx` 徽标、i18n |
