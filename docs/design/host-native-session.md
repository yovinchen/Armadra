...
- `internal/server/native.go`：`NativeOrigin()` = 三个 Tauri 拼写 ∪ 回环 HTTP 来源（`loopbackHTTPOrigin()`：规范化后 scheme 为 `http`、主机是 `localhost` 或回环 IP）；`nativeSession(r, origin, explicit, options)` 判定、`Authorization: Bearer` 解析（恰好一个头、`Bearer ` 前缀）。
...
- Go：`internal/server/native_test.go`（原生来源 + bearer 成功、浏览器来源同票据拒绝、票据二次使用拒绝、HTTPS 形态拒绝原生、bearer 不能用于 Cookie 形态、预检；另有回环 HTTP 壳来源被接受、未列入 `--allow-origin` 的回环来源拒绝、`--public-origin`/TLS 形态拒绝、`NativeOrigin` 拼写矩阵）、`cmd/armadra-host/pair_test.go`（回环 Host 对被允许的 Tauri 与回环 HTTP 来源出票、对浏览器来源仍拒绝）。
...
- `pnpm host:native-session-smoke`：真实 Go Host + 用 CLI 代替桌面壳的取票流程——原生来源 + 票据 → 会话 → `ListDevices`（需 `identity:read`）成功；浏览器来源 + 同一票据 → 拒绝；票据二次使用 → 拒绝；无 bearer 的原生请求 → 401。同一脚本对内核分配端口的回环 HTTP 来源再跑一遍（出票 → 换会话 → `Current` → 单次消费），该段用原始请求，因为 host-client 的原生传输还只认 Tauri 拼写。
...
- 回环 HTTP 壳来源目前只在 Go Host 生效。`packages/host-client`（`NATIVE_PAGE_ORIGINS`、`identity.ts` 的 `nativePageOrigins`）与 `apps/web/src/host/native-session.ts` 仍写死三个 Tauri 拼写，Electron 壳接线时需一并放开；Host 侧无需再改。
- §4.4 描述的仍是 Tauri 壳的取票命令。Electron 壳用等价的 IPC 通道取同一张票，端口由静态服务的内核分配端口决定而非固定 43121。
