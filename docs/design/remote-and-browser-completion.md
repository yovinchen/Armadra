...
>
> **实现载体说明（本次措辞收口，不改设计意图）**：本文正文与 §4「代码布局」、§5「实施拆解」大量出现的 `apps/runtime/src/browser/`、`apps/runtime/src/remote/`、`worker.proto`、`browser.proto`、`Runtime`/`Worker`/`Go Host`、`cargo test` / `go -C apps/host test` / `pnpm protocol:*` 等措辞，是 Rust Runtime + Go Host + Protobuf 跨进程协议架构下的原始设计与实施记录（含大量已完成批次的历史事实）。该架构已随 [core 合一设计](./typescript-core.md) 整体合入 TypeScript core：进程从「壳 / Host / Runtime」三个收敛为「壳 / core」两个，跨进程 Protobuf 协议整体删除，`Runtime`/`Worker` 对应的能力现落在 core 的 `browser/` 与 `remote/` 域，`Host` 对应的能力并入同一个 core 进程。下文只做了少量高影响点的措辞替换，大部分 Rust/Go/proto 细节与批次实施记录原样保留作为历史追溯，不逐条杜撰对应的 TypeScript 文件路径；现状验证命令统一为 `pnpm --filter @armadra/desktop test`（core/桌面壳）与 `pnpm --filter @armadra/server test`（服务器壳），不再有 `cargo test` / `go test` / `pnpm protocol:*`。
...
| 线           | 基线（已提交）                                                                                                                                                           | 本轮目标                                                                                                                                                                         | 不变量                                                                                                       |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| A 受控浏览器 | core 的 `browser/` 域（原 `apps/runtime/src/browser/`）：本机 Chromium 发现、单页 CDP 会话、screencast 经工作空间事件、6 个 Agent 动词、下载暂存、重启恢复（macOS 实机） | 受管二进制、多标签与 frame 寻址、上传/对话框/弹窗、重定向复检、控制租约与徽标、动词补全到设计 §7 全表、专用帧流经服务器壳到手机、Windows/Linux 进程组、SIGKILL 后 profile 锁恢复 | 没有 `eval`、没有裸 CDP；人与 Agent 共用一个 session；缺浏览器时明确不可用，不下载不假装；关闭节点不结束页面 |
| B 远端执行   | core 的 `remote/` 域（原 `apps/runtime/src/remote/`）：SSH 版本锁定握手、文件/搜索/基础 Git 远端执行、2 秒轮询监听、写出后失联 `UNKNOWN_OUTCOME`                         | 仓库面板/文件管理/上传与资产导入远端执行、执行主机可按规则重绑定、远端 notify 事件、协议兼容范围替代精确版本、真实 SSH 认证与 host key 用户确认                                  | 远端工作空间绝不静默回退本机；写出后失联仍是 `UNKNOWN_OUTCOME`；不自动信任 host key；凭据不入库              |

两条线共用一个前提：**远端执行需要一条到控制端的事件通道**（§3.4）。它先服务文件监听；浏览器帧流跨执行主机复用同一通道，但**本轮浏览器 session 仍只运行在控制端主机**（§7）。
...
AI 提交信息是唯一在两台机器上跑的面板动作，所以拆成两步而不是整体代理：`GIT_MESSAGE_CAPTURE = 50` 在持有仓库的那台机器上采集暂存 diff（敏感路径排除、私钥扫描、脱敏都在那边跑），provider CLI 在控制端跑（凭据在那儿），只有生成出来的信息回来，两侧都不写。模型跑完后再向持有仓库的一侧重读一次 source，diff 在生成期间变过就和本机一样判过期。剩下真正只能在控制端跑的（语言服务重启 / 停止）改回 `unsupported_on_remote` 而不是笼统的 `unsupported`：前者能由「切换执行主机」解决，界面得说得出这句话。
...
批次 5 未做：`remote_ssh` 的伪 `ssh` 端到端（打印 `password:` 并调用 `SSH_ASKPASS`）没有写——伪 SSH 运行器是一个丢弃选项的 shell 脚本，它不实现 OpenSSH 的 askpass 协议，要写这条测试得先实现一个假的 `ssh`，那测的是这个假程序而不是 Armadra；`known_hosts`、`prompts`、`askpass` 三个模块的规则改由单元测试覆盖（指纹归属校验、一问一答一次读取、提示文本脱敏、三个环境变量缺一即拒、全部 argv 带 `StrictHostKeyChecking=yes`）。`ssh-keyscan`/`ssh-keygen` 的真实调用路径同理只有 argv 与解析被测，没有连过真实主机。切换的阻塞项现在覆盖编辑器草稿、活动终端、浏览器 session、指向该工作空间的自动化节点与本进程持有的 Git 操作；自动化计划本身由 Host 持有，Runtime 只看得到引用它的节点，所以列出的是节点而不是计划。`stopBlockers` 只结束终端与浏览器 session 这两类进程，草稿与进行中的 Git 操作仍是拒绝——那是重开节点找不回来的东西；停完重新取一次阻塞项，第二次的列表才决定切不切，所以这个开关不会变成「无论如何都切」。上传尚未纳入：控制端没有在飞上传的登记。
...
| 17  | `pnpm --filter @armadra/desktop test`、`pnpm --filter @armadra/server test`、`pnpm --filter @armadra/web test`/`typecheck` 全绿；Windows/Linux 构建通过 | 每批                         |
...
| 独立进程持有 SSH 连接             | 已随 core 合一取消：SSH 连接由同一个 core 进程持有，不再有跨进程转发这一层                        |
