...
| #   | 决策                                                                                                | 依据                                                                                                                                                                                                                                                 |
| --- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | **壳换 Electron，路径仍是 `apps/desktop`，包名仍是 `@armadra/desktop`**                             | 壳只占全仓 1.6%（9,127 行）；`repo.rules.json` 根目录白名单与 `apps/` 命名规则不用改（盘点 §7）                                                                                                                                                      |
| D2  | **终端域留在 Rust Runtime**                                                                         | tmux 已是主后端（契约 §15.3），环境注入、SSH、`session-host` 全在 Runtime，壳从不参与（[终端](../research/nodeterm/terminal-and-tmux.md) §6.3 列了搬去 Electron 的六项代价）                                                                         |
| D3  | **Agent 域留在 Rust Runtime**                                                                       | Hook 客户端是独立进程、汇合点是端点文件，tmux 会话活得比壳久；两个项目独立地从 MCP 退回 CLI 动词面（[Agent](../research/nodeterm/agent-integration.md) §7.2、§7.4）                                                                                  |
| D4  | **持久化继续走 Runtime SQLite**                                                                     | AGENTS.md 的迁移规则正是 SQLite 的强项；nodeterm 选文件是被 git 共享画布 / SSH 项目 / Server Edition 逼的，Armadra 没这三个需求；文件方案并发语义比现有 CAS + `replayLocalEdits` 更弱（[画布](../research/nodeterm/canvas-nodes-and-state.md) §7.4） |
| D5  | **浏览器节点改为 Electron `<webview>`；授权留 Runtime、执行搬 Electron main；删除 screencast 路径** | `<webview>` 是 OOPIF，缩放与命中测试在 Chromium surface 层完成，nodeterm 零补偿代码；Runtime 的 `launch/`（1,373 行）、`cdp.rs`、`stream.rs` 与前端帧流整体作废（[浏览器](../research/nodeterm/browser-node.md) §5）                                 |
| D6  | **明确放弃「手机经 Host 远端看同一页面」**                                                          | 这是唯一实打实的能力损失；今天也只验证到 macOS 本机（status §23）；降级为「截图 + 元素快照 + 看见并撤销租约」控制面，不保留两套渲染路径                                                                                                              |
| D7  | **画布性能修正 P0–P3 与换壳并行，P0 基线必须在 Tauri 壳与 Chrome 各记一次**                         | 卡顿最可能来源是 `useFlowNodes` 订阅整份 `document` 且 `updateNodeData` 必换 document 对象（画布 §7.2），与壳无关。用户决定全部并行（2026-09-19）；P0 在换壳前留下的两份基线是事后二分定位的唯一依据                                                 |
| D8  | **删除 `armadra://` 协议转发、WS 回环转发、原生会话票据整条链、诊断桥**                             | 它们全部因 `tauri://localhost` 不是 HTTP 来源而存在（[平台](../research/nodeterm/process-model-and-platform.md) §6.1）。渲染页由壳的回环 HTTP 静态服务提供，来源是真实的 `http://127.0.0.1:<port>`                                                   |
| D9  | **更新状态机整段移植成 TS，验签安装换 electron-updater**                                            | `updates/machine.rs` 是纯函数十一态，`HostStopFailed`（先停 Host 再装）是 Armadra 独有约束，nodeterm 149 行的 updater 没有它                                                                                                                         |
...
不做的事（本轮明确排除）：Server Edition / 多壳共享 core（Armadra 的 Runtime 已是那个 core）、Windows 实机验收（无 Windows 机器，`crates/session-host` 与 NSIS 打包按离线检查交付）、`send`/`reply` 推送管线、账户级配置目录隔离（Agent §8 批 5，独立于换壳）。
...
- 页面来源是真实 HTTP 来源，`fetch`/`WebSocket` 直连 Runtime 与 Host，**不再需要**协议转发与 WS 回环转发（盘点 §2 第 13、14 项整体删除）。
- Host 侧：`NATIVE_ORIGINS` 三个 `tauri://` 值删除；`handler.go:390`、`native.go`、`auth.go:40`、`serve.go:468` 的原生来源判定改为「回环 HTTP 来源 ∈ `--allow-origin`」。**批次 1 必须先验证** Host 对回环 HTTP 来源是否接受浏览器会话（`identity.browser-session.v1`，cookie）；若 Host 规则要求 HTTPS，则保留 `pair` 票据 Bearer 路径但来源改为 HTTP 来源——此时 Go 侧改动仍只是常量，`docs/design/host-native-session.md` 改状态而非归档。
...
依据 [画布](../research/nodeterm/canvas-nodes-and-state.md) §7.2–§7.3、§8。现有基线（`canvas-react-flow.md:706`）的 30 个终端节点**没有活会话**，测不出用户报的卡顿。
...
| P5  | 保存失败退避 + Retry + 明确文案（核对 `save/autosave.ts` 是否有 nodeterm PR #657 同型缺陷：失败后 `dirty` 能否重新武装防抖）            | 注入一次 Runtime 拒绝，后续编辑仍能触发保存                            |

P0–P3 是收益主体，不依赖 Electron，**换壳批次 1 之前完成**。P4–P5 可与换壳并行。

## 4. 浏览器节点

依据 [浏览器](../research/nodeterm/browser-node.md)。三条照抄的不变量：所有权只在内存、绝不从磁盘恢复；Stop 必须 detach + 撤所有权，只隐藏徽标是 Critical 级 bug；CDP 白名单默认拒绝且连参数一起校验，唯一调用点由结构性测试守住。
...
`docs/design/remote-and-browser-completion.md` 的 §2.1–§2.12 被代码引用数百次（盘点 §5.5），**编号不动**：§2.1 受管二进制、§2.9 跨端画面、§2.10 进程组清理在文首状态行标为「已由 electron-migration §4 取代」，正文保留供回溯；被删代码里的 `设计 §8` 引用随代码消失。`browser_sessions` 表不改列（迁移不可修改），`pid`/`cdp_port` 列留空。
...
| 批   | 内容                                                                                                                                                                 | 验收                                                                                                                                                                                                                                                                                      |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W3.0 | **go/no-go 实验**：独立最小 Electron 工程（`webviewTag: true` + React Flow），两个节点各一个 `<webview>` 指向本地 fixture，fixture 把 `clientX/Y` 与命中元素画到页面 | zoom 0.25/0.5/1/2 四角与中心命中一致；平移后重复；zoom=2 与 0.5 文字是否重新栅格化（截图）；页内滚动、`<select>`、右键、选区、IME 各一次；悬停时 Cmd+滚轮画布不缩放（记录）；拖节点/缩放过程 guest 不重载。**第 3、6 条失败则整个 §4 重估**。结论按探针体例写进 `docs/research/nodeterm/` |
...
| 风险                                                                                       | 处置                                                                                      |
| ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `<webview>` 是 Electron 标记为不推荐的 API；`WebContentsView` 是覆盖层不参与合成，不能替代 | W3.0 先撞；nodeterm 在 Electron 42 上以此为生产路径，跟随其版本                           |
...
| 更新签名密钥体系整体更换                                                                   | 未签名阶段更新器关闭；先签名再接自动更新（nodeterm 同序）                                 |
