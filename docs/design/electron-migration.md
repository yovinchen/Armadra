...
| D8  | **删除 `armadra://` 协议转发、WS 回环转发、诊断桥；原生会话票据链保留、来源判定放宽**               | 前三者因 `tauri://localhost` 不是 HTTP 来源而存在（[平台](../research/nodeterm/process-model-and-platform.md) §6.1）。票据链不能删：回环 HTTP 的 Cookie 无端口隔离（§2.1 已验证）。渲染页由壳的回环 HTTP 静态服务提供，来源是真实的 `http://127.0.0.1:<port>` |
