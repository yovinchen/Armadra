...
3. **`armadra-hook` 命令面**：`<agentId>`（hook 模式，stdin 收 payload，**永远 exit 0**）、`context-usage`（随单会话上下文占用一起移除后保留为静默 no-op，只为老 `settings.json` 不刷屏）、`context <list|summary|transcript|terminal>`、`canvas <verb>`、`browser <17 个动词>`、`doctor`；flag 规则（`--flag value` / `--flag=value` / 裸 flag 为 true / 重复 flag 成数组 / `--flag=value` 是唯一能传 `-` 开头值的形式）与退出码不变。
