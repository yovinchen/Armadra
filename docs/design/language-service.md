> 状态：批次 A、B、C、D 已全部实施（见 §5 表格下的实施状态四行）。本文是 [编辑器与浏览器设计](./editor-browser-design.md) §2「语言服务」与 §4 的实施方案，对应 [功能预期总表](../status/feature-roadmap.md) §3.5 的「语言服务」一行与 [平台实施记录](../status/platform-implementation-status.md) E01 的「LSP」剩余项。本机工作空间已能拉起真实 server，浏览器里出现诊断、hover、补全、格式化与跨文件重命名预览；远端工作空间一律 `UNSUPPORTED`。
...
| 方案                                        | 依赖与体积                                                                                                                                                                               | 多文件 / 传输                                                                                                                                               | 判断                                                                                                                |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `@codemirror/lsp-client`（CodeMirror 官方） | 只依赖 `@codemirror/{state,view,language,autocomplete,lint}`；`vscode-languageserver-protocol` 实测只被当作类型（运行时只 `import` `marked` 与 `@lezer/highlight`）**（已采用，6.2.5）** | `Transport { send(string); subscribe/unsubscribe }` 自定义；`Workspace` 按 uri 管理多个 `EditorView`，自带补全、hover、诊断（lint）、重命名、定义、签名帮助 | **采用**。与已用的 autocomplete/lint 同一作者与版本线，传输层正好套在我们的会话 WS 上                               |
...
**判定结果（2026-09-06）：采用官方包，不写自研客户端。** `language-*.js` = 63.4 kB raw / **19.9 kB gzip**（门槛 150 kB），内容只有 `@codemirror/lsp-client` 与它渲染 hover 文档用的 `marked`；`vscode-languageserver-protocol` 全被当作类型摇掉。它的 `Workspace` 是抽象类，`ArmadraWorkspace` 直接实现了「同一 uri 只由拥有者视图发 didChange」，第二条标准也满足。chunk 只被 `editor/language/core.ts` 的动态 `import()` 引用（`__vite__mapDeps`），不打开编辑器不加载。

一处例外：官方的 `serverDiagnostics()` 会把通知里的 `version` 与它自己给文档的版本号比对，不一致就丢弃；而我们的版本号来自**执行主机**的影子文档（一条会话里的两个视图、一次 server 重启都不会重置它），那个比对永远不成立。所以诊断的接收端换成 `editor/language/diagnostics.ts`：位置换算与官方相同，同时把原样的诊断存进 store 供问题面板用。
...
验证：`pnpm --filter @armadra/web test`（173 个文件 / 1620 个用例）、`typecheck`、`pnpm check`、`pnpm --filter @armadra/shared test`；`cargo test -p armadra-runtime language`（59 个用例）与 `--test language_real -- --ignored`。真实浏览器（私有 Runtime 副本 + 独立 Vite + headless Chrome）：本机 `ruff server` 打开 `.py` → F401 诊断同时进 store 与编辑器行槽、hover 返回规则文档、格式化改草稿并保存写盘（sha256 与磁盘一致）；换成 `tools/probes/mock-lsp.mjs` 后补全回三项（含中文标签）、重命名弹预览并把两个文件写到磁盘；撤销 execute 授权后会话 `unsupported / execution_not_granted`、不开 socket、`ps` 里没有 server 进程。
