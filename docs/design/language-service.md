> 状态：目标设计。本文是 [编辑器与浏览器设计](../editor-browser-design.md) §2「语言服务」与 §4 的实施方案，对应 [功能预期总表](../feature-roadmap.md) §3.5 的「语言服务」一行与 [平台实施记录](../platform-implementation-status.md) E01 的「LSP」剩余项。已有实现只到能力探测（恒为 `unavailable / not_implemented`）；本文所有接口、文件与批次都是待实施内容，不代表已交付。
