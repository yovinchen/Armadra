> 状态：批次 A、B、D 已实施（见 §5 表格下的实施状态三行），批次 C（Web 客户端与 UI）仍是目标设计。本文是 [编辑器与浏览器设计](./editor-browser-design.md) §2「语言服务」与 §4 的实施方案，对应 [功能预期总表](../status/feature-roadmap.md) §3.5 的「语言服务」一行与 [平台实施记录](../status/platform-implementation-status.md) E01 的「LSP」剩余项。本机与远端工作空间都能拉起真实 server 并把诊断、格式化、`WorkspaceEdit` 送到会话 WebSocket；浏览器侧尚未接线。
