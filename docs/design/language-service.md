> 状态：目标设计。本文是 [编辑器与浏览器设计](./editor-browser-design.md) §2「语言服务」与 §4 的实施方案，对应 [功能预期总表](../status/feature-roadmap.md) §3.5 的「语言服务」一行与 [平台实施记录](../status/platform-implementation-status.md) E01 的「LSP」剩余项。已有实现只到能力探测（恒为 `unavailable / not_implemented`）；本文所有接口、文件与批次都是待实施内容，不代表已交付。
...
| 协议            | 新增 `proto/armadra/v1/language.proto`（能力探测、会话、消息信封、编辑应用），`worker.proto` 的 `WorkerRequest/WorkerResponse` 各加 oneof 分支（编号从 24 起），远端用第二条 `worker --stdio --language-link` 连接承载全双工帧；`resources.proto` 加平台组件 `LANGUAGE_SERVER` |
...
`worker.proto` 追加（现有 oneof 最大编号 23）：

| 消息                               | 新分支                                                                                                                                                                                                                                                          |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `WorkerRequest.action`             | `LanguageCapabilitiesRequest language_capabilities = 24; OpenLanguageSessionRequest open_language_session = 25; CloseLanguageSessionRequest close_language_session = 26; LanguageFrame language_frame = 27; LanguageApplyEditRequest language_apply_edit = 28;` |
| `WorkerResponse.result`            | `LanguageCapabilities language_capabilities = 24; LanguageSession language_session = 25; LanguageFrame language_frame = 26; LanguageApplyEditResult language_apply_edit = 27;`（关闭会话应答 `language_session`，state = STOPPED）                              |
...
本机验证资源（2026-09-06 实测）：`typescript-language-server`、`gopls`、`pyright-langserver` 不存在；`rust-analyzer` 只有 rustup 代理且组件未装（正好覆盖「探测失败」分支）；`ruff 0.16.1` 存在，`ruff server` 是真实 LSP（诊断、格式化、代码操作）。因此真实进程验证用 `ruff server`，其余能力用最小 mock：`scripts/probes/mock-lsp.mjs`（Node ≥ 22 是仓库前置；stdio、`Content-Length` 帧；实现 initialize、didOpen/didChange 后对含 `TODO` 的行推诊断、hover 回当前单词、completion 回固定三项、rename 返回跨两个文件的 `WorkspaceEdit`、主动发 `workspace/configuration`；开关 `--crash-after=N`、`--hang`、`--big-response` 用于生命周期与上限测试）。
