...
> 已落地（实施进度 §56）。§3 的共享只发**工作空间上**的授权，而 core 还有一批不挂在 `/api/workspaces/{id}` 下的路由。它们对成员逐条归成三类，判定在 `core/identity/route-access.ts`（服务器壳的路由门），每一行都有路由门单测（`route-access.test.ts` 的「全局路由的权限表」）。桌面壳没有请求身份，整道门放行；owner 恒为全权。

| 类别                   | 路由                                                                                                                               | 成员要什么                                                                                              | 理由                                                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| 按对象落到工作空间     | `POST /api/agent-status/{nodeId}/read`                                                                                             | 节点所在画布的 `canvas:read`                                                                            | 清掉节点头上的未读点，看得见画布的人都会做                                                                         |
|                        | `GET /api/agent-status/{nodeId}/transcript`                                                                                        | `terminal:read`                                                                                         | 转录是终端画面的文字版                                                                                             |
|                        | `POST /api/agent-status/{nodeId}/suggest-title`                                                                                    | `canvas:write`                                                                                          | 结果是一次改名                                                                                                     |
|                        | `GET /api/nodes/{nodeId}/context-reads`                                                                                            | `canvas:read`                                                                                           | 画布上一个节点的历史                                                                                               |
|                        | `POST /api/approvals/{pendingId}/answer`                                                                                           | 审批所在画布的 `approval:answer`（driver）                                                              | 替 Agent 代答权限提示，与 `terminal:drive` 同档（S5）                                                              |
|                        | `POST /api/control/confirm/{requestId}`                                                                                            | 请求所在画布的 `approval:answer`                                                                        | 同意 Agent 关掉一个节点，同样是代答                                                                                |
|                        | `POST /api/terminals`，`/api/terminals/{id}/…`                                                                                     | 创建 `terminal:create`；读画面 `terminal:read`；写自己开的 `terminal:create`、写别人的 `terminal:drive` | 「自己开的」按 `terminal_sessions.creator_principal_id`（迁移 0028）判，core 重启后照旧                            |
|                        | `GET /api/workspaces`                                                                                                              | 放行，答案只留有 `canvas:read` 的                                                                       | 侧栏要列出共享给他的画布                                                                                           |
| 无害的全局读           | `GET /api/agents`、`GET /api/agents/{id}/models`、`GET /api/models/catalog`、`GET /api/terminals/backend`、`GET /api/usage/status` | 被共享了任意一块画布                                                                                    | 新建菜单、节点头、终端面板离不开；只有本机装了哪些 CLI、启动参数、平台与公开状态页，没有环境变量、凭据或别人的数据 |
| 本机管理（只有 owner） | `/api/settings`、`/api/settings/local`、`/api/execution-hosts*`、`/api/ssh/*`、`/api/data/*`                                       | 403                                                                                                     | 整台机器的配置：SSH 主机、执行主机、备份                                                                           |
|                        | `/api/usage*`（除公开状态页）、`/api/conversations*`、`/api/agents/{id}/integration*`、`POST /api/models/catalog/refresh`          | 403                                                                                                     | owner 自己的账户用量、本机所有 CLI 的对话索引、改 CLI 的配置目录                                                   |
|                        | `/api/git/clone*`、`/api/power*`、`/browser/*`、`/api/github/*`、`/api/automations/*`、`/api/terminals/{id}/node-token/*`          | 403                                                                                                     | 写服务器上的任意目录、电源与浏览器控制、owner 的外部账户与自动化、节点令牌                                         |
|                        | 表外的路由（包括已删掉的 `/api/ownership`）                                                                                        | 403                                                                                                     | 「没写要求」在单机上意味着「只有 owner 会来」                                                                      |

对象找不到属于哪块画布（节点从没报过状态、审批已不存在、关闭确认已超时）时一律 403，不泄露它是否存在。

**页面一侧**：设置里读写整台机器的那几页（Agent、集成、终端、工作区、GitHub、SSH、执行主机、数据、账号与用量、快捷键、更新）对成员不列（`nav.ts` 的 `ownerOnly`）；审批按钮与关闭确认只对这块画布的 driver 出现；用量、SSH 提示、键位与主机表不再去问。判定仍在 core，页面只是不把点了必然 403 的入口摆出来。

**组管理员**：组内 `admin` 不编译成任何 scope（S3 不变），只在组管理的几个动作上多一道「你是不是这个组的管理员」：增删本组成员、改本组成员的组内角色、签发与作废只指向本组的邀请。建组、改名、删组、动 owner、签带工作空间的邀请（那是共享，要 `workspace:share`）、别的组，一律 403。组共享因此可以交给组长维护：owner 把工作空间共享给组，组长决定组里有谁。
