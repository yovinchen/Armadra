...
> 状态：目标设计。本文是 [tldraw 画布契约](../contracts/tldraw-canvas-plan.md)的替代方案：把画布引擎从 tldraw 5.4 整体换成 React Flow（`@xyflow/react`，MIT），白板层自写。tldraw 契约的 §N 编号仍被源码注释引用，该文件保留不动；本文实施完成后由收尾批把它标为「已被取代」并移入 `history/`。
...
| `whiteboard/model.ts`（B2，B3 只读）            | `WhiteboardDoc`、`Item`、`Reference`、`whiteboardDocSchema`、`emptyWhiteboard()`                                                                                       |
...
| `whiteboard/migrate/`（B3）                     | `migrateTldrawSnapshot(json, deps): Promise<MigrationResult>`；B0 的 `serialize.parseWhiteboard` 只返回 `{ kind: "tldraw", raw }`，由 `FlowWorkspace` 在 B3 接入后调用 |
...
### 3.2 tldraw 快照 → v2 转换规则

输入是 `boards.whiteboard_json` 里的 `{ store: { id → record }, schema }`（`snapshot.stripDocumentRecords` 的输出格式：只含白板原生记录、绑定与资产，节点 shape 已被剔掉，`link` shape 不在里面，指向节点 shape 的 `arrow` binding 会留着）。

| 编号 | tldraw 记录                                                    | v2 结果                                                                                                                                                                                                                                                                                                         | 报告                     |
| ---- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| M01  | `draw`                                                         | `ink`：`segments[].path` 经移植的 `decodePoints`（§3.2 末）解码，多段拼接，`isPen` 的压力保留，否则 0.5；`scale/scaleX/scaleY` 乘进点；`color/size` 直取，`fill/dash` 丢弃                                                                                                                                      | 否                       |
| M02  | `highlight`                                                    | `ink` + `highlight: true`                                                                                                                                                                                                                                                                                       | 否                       |
| M03  | `text`                                                         | `text`：`richText` 遍历 `doc → paragraph → text` 节点拼纯文本（换行保留），`w`、`textAlign`、`color`、`size`；`font`、`scale` 丢弃（`scale` 乘进字号档位取最近）                                                                                                                                                | 富文本含非文本节点时报告 |
| M04  | `geo`                                                          | `shape`：`rectangle / ellipse / triangle / diamond / hexagon / star` 直取；`oval → ellipse`、`rhombus / rhombus-2 → diamond`；其余 14 种 → `rectangle`；标签取 `richText` 纯文本；`fill`：`none → none`、`semi / pattern → semi`、`solid / fill / lined-fill → solid`；`dash`：`draw / solid → solid`、其余同名 | 非直取的 geo 报告        |
| M05  | `line`                                                         | `line`：`points` 字典按 `index` 排序取 `x/y`；`spline: cubic` 按直线段连接；无箭头                                                                                                                                                                                                                              | spline 报告              |
| M06  | `arrow`，两端都无 `arrow` binding                              | `line`：`start` / `end`；`arrowheadStart/End` 非 `none` 即 `true`；`bend ≠ 0` / `kind: elbow` 取直                                                                                                                                                                                                              | 弯曲 / 肘形报告          |
| M07  | `arrow`，两端绑到白板 shape                                    | `line`：端点 = 目标 shape 包围盒 × `normalizedAnchor`（默认中心）；不再绑定                                                                                                                                                                                                                                     | 报告                     |
| M08  | `arrow`，一端绑节点 shape、一端绑白板 shape                    | `reference { id: meta.armadra.contentId ?? 新 uuid, itemId, nodeId }`；节点 uuid = `toNodeId(binding.toId)`；节点已不存在则丢弃                                                                                                                                                                                 | 节点缺失时报告           |
| M09  | `arrow`，只绑节点一端或两端都绑节点                            | 丢弃（后者不该出现在快照里；前者是半条线）                                                                                                                                                                                                                                                                      | 否                       |
| M10  | `image`                                                        | `image`：`assetId` → 资产记录 `meta.armadra.path` 直取；`crop / flipX / flipY / rotation` 任一存在，或资产只有 data URL：用旧记录构造临时 `image` item 经 `raster.ts` 栅格化后走 `runtimeApi.uploadAsset` 得到新路径；上传失败整张丢弃                                                                          | 栅格化或丢弃时报告       |
| M11  | `note`                                                         | `text` + `style.fill: "solid"`、`color`（便签底色语义）；尺寸 200×200 起按内容自动高                                                                                                                                                                                                                            | 否                       |
| M12  | `frame`（原生，非节点）                                        | `store.addNode("group", { position, size, title: name })`（进 `nodes` 表）；子记录 `parentId` 指向新节点；嵌套 frame 拍平到最近的可用父级（组不能进组）                                                                                                                                                         | 嵌套时报告               |
| M13  | `group`（tldraw 原生分组）                                     | 拍平：子记录坐标加上分组偏移，分组记录丢弃                                                                                                                                                                                                                                                                      | 否                       |
| M14  | `bookmark / embed / video`                                     | 丢弃                                                                                                                                                                                                                                                                                                            | 报告                     |
| M15  | 任何记录的 `rotation ≠ 0`                                      | 置 0，位置取旋转后包围盒左上角                                                                                                                                                                                                                                                                                  | 报告                     |
| M16  | `asset` 记录、`document` / `page` / `instance` 等非 shape 记录 | 不产生 item；资产只在 M10 里被引用                                                                                                                                                                                                                                                                              | 否                       |
| M17  | 未知 shape 类型                                                | 丢弃                                                                                                                                                                                                                                                                                                            | 报告                     |

`z`：按 tldraw 的 `index`（fractional index 字符串）字典序排序后重新编号。颜色名 13 个原样保留。

**墨迹点解码**：tldraw 5.4 的 `draw.segments[].path` 是 base64 编码的 float16 增量点串（`@tldraw/tlschema` 的 `misc/b64Vecs.mjs`，MIT）：首点 16 个 base64 字符（3 维）或 12 个（2 维，`dim: 2`），后续每点 8 个字符，为 float16 小端的 `dx, dy, dz`。把解码半边（约 150 行：base64 → 字节、float16 → number、首点 + 增量累加）移植到 `whiteboard/migrate/tldraw-points.ts`，文件头保留 tldraw Inc. 的 MIT 版权声明。夹具（§5.2 B0 第 0 步）在删除 tldraw 之前用 `compressLegacySegments` 生成，转换测试断言解码回同一组点（误差 < 0.01，float16 精度）。

### 3.3 加载、备份、写回与回退

`whiteboard/serialize.parseWhiteboard(text)` 返回三态：

| 判据                                                            | 结果                                                                                                                                                                   |
| --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 空串                                                            | `{ kind: "empty" }` → `emptyWhiteboard()`                                                                                                                              |
| JSON 且 `engine === "armadra-flow" && version === 2` 且通过 zod | `{ kind: "v2", doc }`                                                                                                                                                  |
| JSON 且有 `store` 与 `schema`                                   | `{ kind: "tldraw", raw }` → 走转换                                                                                                                                     |
| 其余（解析失败、未知 engine、更高 version）                     | `{ kind: "unknown", raw }`：白板按空处理，**原字符串原样保留**；只要用户没有改动白板，保存时继续写回原文；一旦白板被编辑才覆盖，并 toast「旧白板格式无法识别，已覆盖」 |

「未知即保留」让 `tools/canvas-ownership-e2e.mjs` 的字节往返断言（它写入的是一段假 tldraw 结构）与未来更高版本的快照都不会被这一版无声清空。

转换流程（`FlowWorkspace` 打开一块板时，`kind === "tldraw"`）：

1. 备份：把原字符串以 `whiteboard-<boardId>-tldraw.json` 经 `runtimeApi.importFiles(workspaceId, [{ file, path }])` 写进工作区 `.armadra/imports/`（已有接口，落点由 Runtime `imports.rs` 固定）。备份失败 → 转换仍在内存里进行以便显示，但 `whiteboard` 不置脏、保存时继续写回原文，toast 提示并在右键菜单提供「重试转换」。
2. `migrateTldrawSnapshot(raw, { uploadAsset, rasterize })` 生成 `WhiteboardDoc` 与报告 `{ converted, degraded: [{ id, kind, reason }], dropped: [...] }`；M12 的原生 frame 经 `store.addNode` 建组（这一步会置脏 `nodes`）。
3. `store.setWhiteboard(doc, { history: "ignore" })` 并置脏 → 下一次自动保存写回 v2；`legacy` 字段记录备份路径与原文摘要。
4. 报告非空时 toast 一条汇总（「已转换 N 项，M 项降级，K 项丢弃，原文已备份到 …」），详情写 `console.info` 一次；不弹对话框。

回退（用户退回 tldraw 构建）：旧版 `parseWhiteboard` 遇到 v2 JSON 返回 `null`（它只认 `store` + `schema`），白板显示为空，节点与连线不受影响；备份文件可手工恢复。反向不需要迁移。
...
canvas/
  ...
  whiteboard/                  §2.4
    ...
    nodes/ tools/
    migrate/tldraw.ts          M01–M17 主流程
    migrate/tldraw-points.ts   b64 float16 解码（MIT 声明）
    migrate/tldraw-richtext.ts 富文本 → 纯文本
    migrate/fixtures/*.json    B0 第 0 步生成
  interaction/
    tool-store.ts              F21
...
| B1  | 连线 / Frame / 整理 / 覆盖层 / 缩略图 | 与 B2、B3 并行 | B0         | 约 1,300 行                  | `canvas/flow/edges/**`、`canvas/flow/nodes/{GroupNode,ConnectionHandles}.tsx`、`canvas/flow/Minimap.tsx`、`canvas/flow/overlays/**`、`canvas/tidy-flow.ts`（B0 只放签名）、`canvas/connection.ts`、`panels/github/FrameReferenceBadges.tsx`、`app/minimap-preferences.ts`                                                                                                                                                                                                                                                                                                   |
| B2  | 白板层                                | 与 B1、B3 并行 | B0         | 约 2,400 行                  | `canvas/whiteboard/**`（除 `migrate/`）、`canvas/dnd/**`、`canvas/assets.ts`、`canvas/interaction/pointer.ts`、`keybindings.ts`（只加三条）、`i18n/canvas.ts`（只加 `clipboard.*`、`style.*` 段）                                                                                                                                                                                                                                                                                                                                                                           |
| B3  | tldraw 快照转换                       | 与 B1、B2 并行 | B0         | 约 900 行                    | `canvas/whiteboard/migrate/**`、`canvas-ownership/mapping.ts`（一个常量）、`i18n/canvas.ts`（只加 `migrate.*` 段）                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| B4  | 菜单 / 偏好 / Dock / 手机 / 清理      | 与 B5 并行     | B1、B2、B3 | 约 1,100 行；删除约 600 行   | `canvas/menus/**`、`shell/ControlsCluster.tsx`、`panels/settings/pages/WhiteboardPage.tsx`、`app/preferences/**`、`app/preferences-store.ts`、`app/commands.ts`、`shell/MobileFocusPage.tsx`（如需）、`platform/layout.ts`（只读）、`i18n/{canvas,commands,mobile}.ts` 其余部分                                                                                                                                                                                                                                                                                             |
...
| B6  | 验收、性能、打包、文档                | 串行，单 Agent | B4、B5     | 文档为主                     | `docs/guides/**`、`docs/status/**`、`docs/contracts/tldraw-canvas-plan.md`（只改状态行并移入 `history/`）、`README.md`、`apps/web/README.md`、`docs/README.md`                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 批  | 输入                                                                                                                                               | 输出                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 通过标准                                                                                                                                                                                                                                                                                                                                                                                                |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B0  | §2.1–§2.3、§2.7–§2.9、§2.11、§4；现 `TldrawWorkspace.tsx`、`sync/`、`store/canvas/`                                                                | **第 0 步**（删依赖之前）：一次性 vitest 脚本用 tldraw 的 `compressLegacySegments`、`toRichText` 与真实 store 记录生成 `whiteboard/migrate/fixtures/{draw,highlight,text,geo,arrow,line,note,image,frame,group,rotation}.json`，并从本机 `canvas.db` 抽一份真实快照（脱敏后）作 `real-board.json`。随后：依赖替换；`FlowWorkspace`；`flow/*`；`sync/project.ts`；`store/canvas/*` 去 editor；`history.ts`；`whiteboard/{model,serialize}.ts` 骨架（v2 空文档、三态解析、序列化，`kind: "tldraw"` 暂按未知保留原文）；`NodeShell` 改造；Dock / DockTools 最小改造（工具组只显示选择 / 手，其余按钮禁用直到 B2）；`canvas.css` 重写；测试迁移（§6.1 标 B0 的行）。tldraw 依赖的其它文件（`content-links.ts`、`create-content-reference.ts`、`dnd/external-content.ts`、`assets.ts`、`shape-menu.tsx`、`StylePanel.tsx`、`CanvasPreferencesMenu.tsx`、`use-tldraw-preferences.ts`、`WhiteboardPage.tsx`）在 B0 里**先降到最小可编译版本**（导出名与签名保留，实现返回空 / no-op，文件头写 `// B<N> 重建`），不允许留下任何 `tldraw` import | 7 种节点在浏览器里可建、拖（只认头部）、resize、折叠、最大化、分组加入 / 离开、删除（有会话弹确认）；终端点击 0 字节进 PTY、滚轮走 tmux 桥、⌘滚轮缩放；撤销 / 重做拖动一步一条，Agent 经 `open-agent` 建的节点不被 ⌘Z 撤掉；自动保存与 409 变基通过 `canvas-store.test.ts` / `autosave.test.ts`；全树 `grep -rE "tldraw" apps/web/src` 为 0；`pnpm --filter @armadra/web build` 通过且无 `tldraw` chunk |
...
| B3  | §3.2、§3.3；B0 生成的夹具                                                                                                                          | `migrate/tldraw.ts`、`tldraw-points.ts`、`tldraw-richtext.ts` 与测试；`FlowWorkspace` 接入转换流程（备份 → 转换 → 置脏 → 报告）；`mapping.ts` 常量                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 夹具逐项断言 M01–M17；`real-board.json` 转换后 items 数、references 数与人工清点一致；用真实旧库启动：备份文件出现在 `.armadra/imports/`，画布上墨迹 / 文字 / 图片 / 引用位置与 tldraw 构建截图对照偏差 ≤ 2px；再次刷新不重复转换、不重复备份；`pnpm canvas:e2e` 的假快照字节不变                                                                                                                       |
...
- `FlowWorkspace.tsx` 归 B0；B1–B5 通过 B0 预留的挂载点接入：`nodeTypes` / `edgeTypes` 常量文件、`<ToolLayer />` 插槽、`<CanvasMenus />` 插槽、`onBoardOpened(document)` 钩子（B3 的转换流程）。
...
| T09  | 纯函数 | M01–M17 逐项夹具；`tldraw-points` 与 `compressLegacySegments` 夹具误差         | B3  |
| T10  | 组件   | `useContentLinks` 对 `references` 变化发布 pending → ready；删除对象撤销引用   | B5  |
| T11  | 集成   | 打开含 tldraw 快照的文档 → 备份调用一次 → `whiteboard` 置脏 → 保存体为 v2      | B3  |
...
| 编号 | 风险                                                                       | 影响                    | 缓解                                                                                                       | 回退                                  |
| ---- | -------------------------------------------------------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| R01  | 墨迹解码移植与 tldraw 实际编码不一致（float16、2D/3D 首点长度）            | 旧墨迹错位或丢失        | B0 第 0 步的夹具在删依赖前用真实 `compressLegacySegments` 生成；`real-board.json` 人工对照；备份文件永远在 | 用户可从备份手工恢复；本批不合入      |
...
| R07  | 备份走 `importFiles`，工作区不可写或远端工作空间时失败                     | 转换不落盘              | §3.3 第 1 步：失败则不置脏、保留原文、可重试                                                               | 原文永远保留                          |
