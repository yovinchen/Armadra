...
| B4  | 菜单 / 偏好 / Dock / 手机 / 清理       | 与 B5 并行     | B1、B2 | 约 1,100 行；删除约 600 行   | `canvas/menus/**`、`shell/ControlsCluster.tsx`、`panels/settings/pages/WhiteboardPage.tsx`、`app/preferences/**`、`app/preferences-store.ts`、`app/commands.ts`、`shell/MobileFocusPage.tsx`（如需）、`platform/layout.ts`（只读）、`i18n/{canvas,commands,mobile}.ts` 其余部分                                                                                                                                                                                                                                                                                             |
