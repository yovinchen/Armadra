...
> 范围：`apps/web/src/canvas/` 及画布外的 tldraw 依赖、`boards.whiteboard_json` 的新格式与客户端一次性转换、实施批次与验收。core、数据库迁移一概不改（§3.5 给出核实结果，写作时对应的是 Runtime/Host 分进程年代的源码）。
...
> 本文写作时执行层是 Rust Runtime、服务层是 Go Host；二者已合并进统一的 TS core（`apps/desktop/src/core/`，见 [typescript-core.md](typescript-core.md)）。下文的 Runtime / Host 提法是当时的实现分工，读作 core 的对应模块即可；本文验证过「画布改动不影响 Runtime/Host」的结论，现状即「不影响 core 中对应的域」。
...
| 保留     | 7 种节点与节点体代码、`canvas-store` 全部动作签名、`nodes` / `edges` 表与 core 接口、上下文连线的贴边贝塞尔与箭头语义、内容引用（白板对象 → Agent，`ContextLink.kind = "shape"` 契约不变）、Frame↔worktree 绑定、整理排布、锁定、Esc 回选择、缩略图状态描边、右键三套菜单、拖放与粘贴规则、白板偏好的大部分、快捷键表、手机焦点页。                                                                                                                                                       |
...
| 数据契约 | `whiteboard_json` 仍是**不透明、带版本的 JSON 字符串**（§3.1 v2 格式，`{"engine":"armadra-flow","version":2,…}`）。core 只看长度与摘要，已逐文件核实（§3.5）；不新增迁移。旧 tldraw 快照在客户端加载时一次性转换，下一次保存写回；转换前先把原文备份到工作区 `.armadra/imports/`（§3.3）。                                                                                                                                                                                                 |
| 归属标识 | 右下角的「React Flow」链接**不显示**（`proOptions.hideAttribution`，2026-09-14 用户决定；MIT 许可不要求它）。右下角整个是缩略图的，离两边各 14px。                                                                                                                                                                                                                                                                                                                                         |
...
| 不做     | 多人协同；白板对象的实时合并（保存冲突时白板整块以本地为准，沿用 `save/canvas-save-queue.ts` 的既有局限）；core 解析白板；把墨迹渲染搬进 core 之外的地方；手机上的绘图工具（只保留平移、缩放、选择、焦点页）。                                                                                                                                                                                                                                                                             |
...
| 不受影响     | `nodes/*Node.tsx` 节点体、`terminal/`、`packages/shared`（`whiteboard` 字段、`contextLinkSchema`）、`apps/desktop/src/core`、`tools/canvas-ownership-e2e.mjs`（白板按字节往返）                                                                                                                                    | 只有注释里的「tldraw 计划 §N」引用                                                  |
...
| F21  | Dock 工具组：选择 / 手 / 笔（下拉：画笔 / 高亮）/ 形状（下拉）/ 线（下拉：直线 / 箭头）/ 文字 / 引入；当前工具高亮；键 V/H/D/… | `tools.ts` 表 + `editor.setCurrentTool`；`DockTools` 用 `useValue`                                  | `tools.ts` 表 + `DOCK_TOOL_ITEMS` 排布（成对的工具合成一格，当前项记在 `tool-store.getToolGroupChoice`）；画框工具已删除，新建画框留在 Dock「新建」菜单；「引入」开不限类型的文件选择；工具状态在 `interaction/tool-store.ts`（`useSyncExternalStore`），Dock / 快捷键 / 命令面板同一入口                                                                     | B2   |
...
- 内容引用的数据流不变：`content-links.ts` 的 `useContentLinks()` 改成订阅 `whiteboard.references` 与 `items`，`shapeSignature` 换成 item JSON（去掉 `x/y`），`resolveContent` 换成 `raster.ts` 与 item 文本；`ContextLink.content { status, sourceShapeId, shapeType, text, textTruncated, pngPath }` 一个字段不改（core 中承接原 `collab/context_link.rs` 的逻辑不动）。`sourceShapeId` 填 `wb:<uuid>`，`shapeType` 填 `ink / text / shape / image / line / frame`（新值只是提示，core 不按它分支——已核实 `read_shape` 只读 `text` / `png_path`）。
...
> 本表核实的是 Rust Runtime / Go Host 分进程年代的源码（`apps/runtime`、`apps/host`、`proto/` 均已在合并为 core 时删除，逻辑并入 `apps/desktop/src/core/` 对应域）；结论——`whiteboard_json` 对执行/服务层不透明——随迁移原样保留，未在合并中改动。
...
结论：**不改契约、不加迁移、不改 core 中承接 Host/Runtime 逻辑的源码**。`packages/shared` 的 `boardSchema.whiteboard` 注释可在 B4 顺手改成「不透明白板文档（§3.1）」。
...
通用验收命令（每批都跑）：`pnpm --filter @armadra/web typecheck`、`pnpm --filter @armadra/web test`、`pnpm repo:check`、`pnpm format:check`；触及 `packages/shared` 注释以外内容时加 `pnpm --filter @armadra/shared test`；B3 / B5 加 `pnpm --filter @armadra/desktop test`（证明契约未变，预期无改动）与 `pnpm canvas:e2e`（白板字节往返）。真实浏览器检查按 §6.3 中标了该批编号的项。
