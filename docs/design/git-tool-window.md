...
- **交互式 rebase 与 cherry-pick 的对话框是搬过来的**，不是重写的
  ...
  一个复制按钮放在表单上方。
- **两件旧能力换了地方，不是没了**：新建 worktree（连同「同时建一个绑定的
  Frame」与初始化脚本）挂在分支树 `Worktree` 组的右键上——Frame 仍然要等这条
  检出真的出现在分支树里才建，凭据是列表本身；`git init` 挂在提交页的空状态
  上，因为那是整个界面里唯一说得出「这个工作空间还不是仓库」的地方。
