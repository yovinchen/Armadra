...
- **状态机**（`core/terminal/hibernator.ts`）：巡检每 60 秒一轮。`idle` 是判据全过、只差时间；`hibernate-requested` 时先礼后兵：在 CLI 的输入框里敲它自己的退出命令（注册表的 `exitCommand`：Claude、OpenCode、Oh My Pi、Copilot 是 `/exit`，Codex、Pi 是 `/quit`；命令与回车分两次写，免得被当成粘贴），前台变回 shell 就接着往下走，最多等 5 秒；然后调 `manager.hibernate`，后端 `terminate` 返回之后才把行记成 `status = 'terminated'`、`termination_intent = 'hibernate'` 并发 `terminal.hibernation`，抛了就退回 `running`。不另开表：恢复要的其余几样（cwd、shell、Agent、`agent_status.session_id`、节点数据里的权限模式与模型）都在库里，进程不在时没有东西会改写它们。
...
没做：无原生循环与浏览器控制依赖两条判据没有可观测的信号，没有实现；Windows 上进程树为空，后台作业判据如实答「没看到」。
...
**清空、多组替代键与自定义 `when`**：每行的「更多」菜单里可以再录一组替代键（追加，修饰键别名归一后重复的不加）、逐组移除、设为无，以及编辑条件。「设为无」存成一条空串覆盖——没有这个键是「没覆盖」，空串是「覆盖为空」，来源照报本层，↺ 能退回下一层。条件存在每层不分平台的 `when` 表里（`settings.keymap.when` / `profiles.<id>.when`，本设备同形），空串表示「不设条件」；输入框按 `when.ts` 的语法校验，语法错与不认识的键分开提示，有错不能保存；读设置或导入时读不懂的条件直接丢掉。冲突检测逐组比较替代键，并用合并之后的条件判断能否同时成立。节点内的监听器（编辑器、浏览器）不传键位表时读 `setActiveKeymap` 那一份，用户改过的键和条件在节点里同样生效。
