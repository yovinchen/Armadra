...
- 节点操作：`<NodeResizer>`（选中时显示，最小尺寸表见 §3.4）；折叠到 40px 头部（xterm 保持挂载）；最大化改真实 rect（终端因此获得更多行）并记录 `premaxRect`；Focus 层把节点 DOM 移到 `fixed inset-0 z-40`，Esc 不退出（留给 CLI），用工具簇按钮退出。
...
- 行为：默认视图 Canvas/Kanban、Grid size 24、默认节点尺寸 640×440、Snap to grid、Snap to grid mode、Pan-hover delay 600ms、双击聚焦、Markdown 预览、侧栏默认折叠、侧栏分组方式 工作空间/状态。
