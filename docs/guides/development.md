...
| `VITE_TLDRAW_LICENSE_KEY`                       | 构建时注入的 tldraw 许可证密钥。tldraw 5 在非开发来源（打包桌面的 `tauri://localhost`、正式域名）上没有密钥时会在挂载 5 秒后卸掉编辑器；浏览器开发（`http://` 回环）不受影响。密钥从 tldraw.dev 获取（有免费试用与 hobby 许可），CI 从 secret `TLDRAW_LICENSE_KEY` 注入 |
| `ARMADRA_DESKTOP_DIAGNOSTIC_WS`                 | 仅 debug 构建的桌面壳：`ws://127.0.0.1:<port>`，页面的 console.error/warn、未捕获异常、fetch 结果、WebSocket 生命周期与 DOM 探针逐条发到这个回环 WebSocket（打包后的 WKWebView 没有开发者工具）；`ARMADRA_DESKTOP_DIAGNOSTIC_PRELUDE` 是在页面脚本之前执行的一段 JS     |
