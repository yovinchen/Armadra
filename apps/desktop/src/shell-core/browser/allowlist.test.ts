...
describe("the allowlist is default deny", () => {
  ...
  it("holds no method from a forbidden domain", () => {
    for (const domain of FORBIDDEN_DOMAINS) {
      const leaked = ALLOWED_METHODS.filter((method) => method.startsWith(`${domain}.`));
...
describe("Page.navigate", () => {
  it("takes http and https", () => {
    expect(isAllowed("Page.navigate", { url: "https://example.com/a?b=1" })).toBe(true);
    expect(isAllowed("Page.navigate", { url: "http://127.0.0.1:8080/" })).toBe(true);
  });
...
describe("Runtime.callFunctionOn", () => {
  ...
  it("requires the answer to come back by value", () => {
    expect(isAllowed("Runtime.callFunctionOn", { ...base, returnByValue: false })).toBe(false);
  ...
  it("takes at most one scalar argument", () => {
    expect(isAllowed("Runtime.callFunctionOn", { ...base, arguments: [{ value: 7 }] })).toBe(true);
    expect(isAllowed("Runtime.callFunctionOn", { ...base, arguments: [{ value: "x" }] })).toBe(true);
    expect(
      isAllowed("Runtime.callFunctionOn", { ...base, arguments: [{ value: 1 }, { value: 2 }] }),
    ).toBe(false);
    expect(
      isAllowed("Runtime.callFunctionOn", { ...base, arguments: [{ objectId: "1.2.3" }] }),
    ).toBe(false);
    expect(
      isAllowed("Runtime.callFunctionOn", {
        ...base,
    ...
    expect(
      isAllowed("Runtime.callFunctionOn", { ...base, arguments: [{ value: { a: 1 } }] }),
    ).toBe(false);
  });

  it("refuses awaitPromise and userGesture", () => {
    expect(isAllowed("Runtime.callFunctionOn", { ...base, awaitPromise: true })).toBe(false);
    expect(isAllowed("Runtime.callFunctionOn", { ...base, userGesture: true })).toBe(false);
  });
});
...
describe("DOM.getDocument", () => {
  ...
  it("refuses the whole tree and refuses piercing frames", () => {
    ...
    expect(isAllowed("DOM.getDocument", { depth: 0, pierce: true })).toBe(false);
  });
});
...
describe("input", () => {
  it("bounds a mouse event by the measured viewport", () => {
    ...
    const at = (x: number, y: number) =>
      isAllowed("Input.dispatchMouseEvent", { type: "mousePressed", x, y }, viewport);
  ...
  it("refuses a key event carrying text", () => {
    expect(isAllowed("Input.dispatchKeyEvent", { type: "keyDown", key: "Enter" })).toBe(true);
    expect(
      isAllowed("Input.dispatchKeyEvent", { type: "keyDown", key: "Enter", text: "x" }),
    ).toBe(false);
    expect(
      isAllowed("Input.dispatchKeyEvent", { type: "keyDown", key: "Enter", unmodifiedText: "x" }),
    ).toBe(false);
  });

  it("only takes keys from the closed list", () => {
    for (const key of BROWSER_KEYS) {
      expect(isAllowed("Input.dispatchKeyEvent", { type: "keyDown", key }), key).toBe(true);
    }
    for (const key of ["a", "F12", "Meta", "ContextMenu"]) {
      expect(isAllowed("Input.dispatchKeyEvent", { type: "keyDown", key }), key).toBe(false);
    }
  });
  ...
  it("only takes the two editing commands", () => {
    for (const command of BROWSER_COMMANDS) {
      expect(
        isAllowed("Input.dispatchKeyEvent", { type: "rawKeyDown", commands: [command] }),
        command,
      ).toBe(true);
    }
    expect(
      isAllowed("Input.dispatchKeyEvent", { type: "rawKeyDown", commands: ["paste"] }),
    ).toBe(false);
  });

  it("insertText is text and only text", () => {
    expect(isAllowed("Input.insertText", { text: "hello" })).toBe(true);
    expect(isAllowed("Input.insertText", { text: "x".repeat(4_097) })).toBe(false);
    expect(isAllowed("Input.insertText", { text: "hi", commands: ["selectAll"] })).toBe(false);
  });
});
...
describe("capture", () => {
  it("takes a clip of numbers and nothing else", () => {
    ...
    expect(
      isAllowed("Page.captureScreenshot", { clip: { x: 0, y: 0, width: 10, height: "10" } }),
    ).toBe(false);
