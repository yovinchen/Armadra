...
const RESOLVE_SELECTOR =
  'function (selector) { var el = null; try { el = document.querySelector(selector); } catch (e) { return { found: false, invalid: true }; } if (!el) return { found: false }; var st = getComputedStyle(el); var r = el.getBoundingClientRect(); var tag = el.tagName.toLowerCase(); var role = el.getAttribute("role") || (tag === "a" ? "link" : tag); var label = (el.getAttribute("aria-label") || el.getAttribute("placeholder") || (tag === "input" || tag === "textarea" || tag === "select" ? "" : el.textContent) || "").replace(/\\s+/g, " ").trim().slice(0, 120); return { found: true, role: role, name: label, x: r.x, y: r.y, w: r.width, h: r.height, visible: st.display !== "none" && st.visibility !== "hidden" && r.width > 0 && r.height > 0, disabled: !!el.disabled, viewportWidth: document.documentElement.clientWidth, viewportHeight: document.documentElement.clientHeight }; }';
