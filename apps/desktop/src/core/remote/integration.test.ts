...
// 这组把本机当执行主机：Worker 在远端开 unix socket 当 Hook 中继。执行主机
// 只会是 POSIX 机器，Windows 上 `listen` 一个文件路径答 EACCES，那里没有这件事；
// 控制端的同步与转发逻辑由其余用例在三个平台上覆盖。
describe.skipIf(process.platform === "win32")(
  "canvas injection on an SSH terminal",
  () => {
    let stateDir: string;
    let dataDir: string;
    let bin: string;
    ...
    it("syncs the artifacts once, keyed by their hashes", async () => {
      ...
      expect(readFileSync(token, "utf8")).toBe(
...
`,
    ...
    it("reopens the relay after the worker reconnects", async () => {
      ...
      const endpoint = new Map(remoteEnv).get(
