...
export async function run(): Promise<number> {
  ...
  const endpoint =
    loaded !== undefined && "ok" in loaded ? loaded.ok : undefined;
  if (loaded === undefined) out("endpoint load: skipped (no path)");
  else if (endpoint === undefined)
    out(`endpoint load: failed (${(loaded as { error: string }).error})`);
  else {
    out(
  ...
  if (candidates.length === 0)
    out("candidates: none (no endpoint is advertised anywhere)");
  else
    out(
  ...
  if (nodeId === undefined) out("tokens: unknown (ARMADRA_NODE_ID is not set)");
    ...
    out("tokens: unknown (endpoint file did not load)");
  else {
    out(
...
async function verify(
  ...
  for (const candidate of candidates) {
    ...
    const token =
      nodeId === undefined ? undefined : nodeToken(candidate, nodeId);
