 ...
 * `<ReferenceSubmenu sourceId={…} />` 放进它留的插槽即可，不需要知道引用
 * 是怎么建的。
 *
...
export interface ReferenceSubmenuProps {
  /**
   * 命中的来源 id：白板对象（`wb:<uuid>` 或裸 uuid 都收）或一个 Frame 的
   * 节点 id。Frame 引用的是它圈住的那一片（`canvas/frame-reference.ts`）。
   */
  sourceId: string;
}

/**
 * 白板对象与 Frame 的右键菜单里的「引用到 Agent」子菜单。
 *
 ...
 */
export function ReferenceSubmenu({ sourceId }: ReferenceSubmenuProps) {
  const t = useT();
    ...
    <ContextMenuSub>
      ...
      <ContextMenuSubContent>
        {targets.map((node) => (
          <ContextMenuItem
            key={node.id}
            onSelect={() => createContentReference(sourceId, node.id)}
          >
