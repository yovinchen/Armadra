...
vi.mock("./content-links", () => ({ MAX_LINKS: 64, useContentLinks: () => ({}) }));
...
function document(title = "Original") {
  return { board: { id: "board" }, nodes: [
    { id: "agent", type: "terminal", title: "Agent" },
    { id: "note", type: "sticky", title },
  ], edges: [{ source: "agent", target: "note" }] } as never;
}

beforeEach(() => {
  vi.useFakeTimers();
  useCanvasStore.setState({ workspace: { id: "workspace" } as never, document: document() });
});
afterEach(() => { vi.restoreAllMocks(); vi.useRealTimers(); });

it("retries failed publication without waiting for a document edit", async () => {
  const put = vi.spyOn(runtimeApi, "putContextLinks").mockRejectedValueOnce(new Error("offline")).mockResolvedValue({} as never);
...
it("serializes requests so a slow previous PUT cannot overwrite a newer reference", async () => {
  ...
  const put = vi.spyOn(runtimeApi, "putContextLinks")
  ...
  await act(async () => { await vi.advanceTimersByTimeAsync(450); });
  expect(put).toHaveBeenCalledTimes(2);
  expect(put.mock.calls[1]?.[2]).toEqual([{ id: "note", title: "Newest", kind: "sticky" }]);
  unmount();
});
...
it("bounded retry can be explicitly restarted, and unmount stops further requests", async () => {
  ...
  const put = vi.spyOn(runtimeApi, "putContextLinks").mockRejectedValue(new Error("offline"));
  const { unmount } = renderHook(() => usePublishContextLinks());
  await act(async () => { await vi.advanceTimersByTimeAsync(30000); });
  expect(put).toHaveBeenCalledTimes(4);
  expect(error).toHaveBeenCalledOnce();
  const options = error.mock.calls[0]?.[1];
  expect(options).toMatchObject({ id: "reference-publish-workspace-board", action: { label: expect.any(String), onClick: expect.any(Function) } });
  put.mockResolvedValue({} as never);
  act(() => window.dispatchEvent(new Event("armadra:refresh-content-references")));
  await act(async () => { await vi.advanceTimersByTimeAsync(450); });
