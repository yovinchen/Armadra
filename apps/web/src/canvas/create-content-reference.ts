...
export function referenceCountForNode(
  ...
  for (const shape of editor.getCurrentPageShapes()) {
    if (shape.type === "link") {
      ...
      const content = contentArrowEnds(shape.id, bindings, (id) =>
        editor.getShape(id),
      );
      if (content && `shape:${content.nodeId}` === nodeId)
        peers.add(content.shapeId);
      if (!content && bindings.some((binding) => binding.toId === nodeId)) {
        for (const binding of bindings) {
          if (
            binding.toId !== nodeId &&
            isNodeShapeRecord(editor.getShape(binding.toId))
          )
            peers.add(binding.toId);
        }
      }
    }
  }
...
export function createContentReference(
  ...
  if (
    !isContentShape(source) ||
    !referenceTargets(editor).some((target) => target.id === targetId)
  )
    return null;
  for (const shape of editor.getCurrentPageShapes()) {
  ...
  editor.run(() => {
    editor.createShape({
      ...
      y: 0,
      props: {
        start: { x: right ? from.maxX : from.minX, y: from.center.y },
    ...
    for (const [terminal, toId, x] of [
      ["start", sourceId, right ? 1 : 0],
      ["end", targetId, right ? 0 : 1],
    ] as const) {
