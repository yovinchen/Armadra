import { MAX_IMPORT_BATCH_BYTES, MAX_IMPORT_FILE_BYTES, MAX_IMPORT_FILES } from "@armadra/shared";
...
export interface FolderSource { name: string; read: () => Promise<BrowserFolder> }
export class FolderReadError extends Error {
  constructor(public readonly reason: "onlyFolders" | "limit" | "invalidPath" | "unavailable") { super(reason); }
}
...
export interface BrowserEntry {
  ...
  createReader?: () => { readEntries: (success: (entries: BrowserEntry[]) => void, error: (error: DOMException) => void) => void };
}
...
function validPath(path: string): void {
  ...
  if (path.length > 4000 || parts.length > 64 || /[\\:\0]/.test(path) || parts.some((part) => !part || part === "." || part === "..")) {
...
function collector(name: string) {
  ...
  return {
    ...
    directory(path: string) {
      ...
      if (result.directories.length >= MAX_IMPORT_FILES) throw new FolderReadError("limit");
      names.add(path); result.directories.push(path);
    },
    file(path: string, file: File) {
      validPath(path);
      if (names.has(path)) throw new FolderReadError("invalidPath");
      if (result.files.length >= MAX_IMPORT_FILES || file.size > MAX_IMPORT_FILE_BYTES || size + file.size > MAX_IMPORT_BATCH_BYTES) {
        throw new FolderReadError("limit");
      }
      names.add(path); size += file.size; result.files.push({ file, path });
    },
  };
}

export async function readDirectoryEntry(root: BrowserEntry): Promise<BrowserFolder> {
  if (!root.isDirectory || root.isFile) throw new FolderReadError("onlyFolders");
  ...
  async function walk(entry: BrowserEntry, path: string): Promise<void> {
    ...
    if (entry.isDirectory && entry.createReader) {
      ...
      while (true) {
        const entries = await new Promise<BrowserEntry[]>((resolve, reject) => reader.readEntries(resolve, reject));
...
export async function readDirectoryHandle(root: DirectoryHandle): Promise<BrowserFolder> {
  const output = collector(root.name);
...
export function foldersFromFileList(files: readonly File[]): BrowserFolder[] {
  ...
  for (const file of files) {
    ...
    if (!output) { output = collector(name); groups.set(name, output); directories.set(name, new Set()); }
    for (let length = 1; length < parts.length; length++) {
      const directory = parts.slice(0, length).join("/");
      if (!directories.get(name)!.has(directory)) { output.directory(directory); directories.get(name)!.add(directory); }
    }
...
export function droppedFolders(data: DataTransfer): FolderSource[] {
  const entries = Array.from(data.items ?? []).filter((item) => item.kind === "file")
    .map((item) => {
      const compatible = item as DataTransferItem & { getAsEntry?: () => FileSystemEntry | null };
      return (compatible.getAsEntry?.() ?? item.webkitGetAsEntry?.()) as unknown as BrowserEntry | null;
    });
  if (entries.length && entries.every(Boolean)) {
    return entries.map((entry) => ({ name: entry!.name, read: () => readDirectoryEntry(entry!) }));
  }
  const files = Array.from(data.files ?? []);
  return foldersFromFileList(files).map((folder) => ({ name: folder.name, read: async () => folder }));
}
...
export async function chooseBrowserFolder(): Promise<FolderSource[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    input.type = "file"; input.multiple = true; input.webkitdirectory = true; input.hidden = true;
    input.addEventListener("cancel", () => { input.remove(); resolve([]); }, { once: true });
    input.addEventListener("change", () => {
      const entries = Array.from(input.webkitEntries ?? []) as unknown as BrowserEntry[];
      const files = Array.from(input.files ?? []); input.remove();
      try {
        if (entries.length) resolve(entries.map((entry) => ({ name: entry.name, read: () => readDirectoryEntry(entry) })));
        else resolve(foldersFromFileList(files).map((folder) => ({ name: folder.name, read: async () => folder })));
