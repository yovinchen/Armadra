...
describe("project folder imports", () => {
  it("routes desktop drops only to the project region, including a Sheet", async () => {
    ...
    openDirectory
      .mockResolvedValueOnce({ id: "first", name: "first" })
      .mockRejectedValueOnce(new Error("not a directory"))
      .mockResolvedValueOnce({ id: "third", name: "third" });
    act(() =>
      nativeDrop(["/projects/first", "/projects/file.txt", "/projects/third"], {
        x: 10,
        y: 10,
      }),
    );
    ...
    expect(openDirectory.mock.calls[0]?.[0]).toMatchObject({
  ...
  it("imports empty browser folders as copies and reports plain files separately", async () => {
    ...
    fireEvent.drop(screen.getByLabelText("Projects"), {
      dataTransfer: {
        types: ["Files"],
        files: [],
        items: [file, directory].map((entry) => ({
          kind: "file",
