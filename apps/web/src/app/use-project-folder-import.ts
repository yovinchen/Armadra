...
import { chooseBrowserFolder, droppedFolders, FolderReadError, type FolderSource } from "../platform/folder-import";
...
export function isProjectDropPoint(point: { x: number; y: number }): boolean {
  ...
  return Boolean(target?.closest("[data-project-drop-zone]")) && !target?.closest("input, textarea, [role='menu']");
}
...
export function useProjectFolderImport() {
  ...
  const report = useCallback((error: unknown, name?: string) => {
    const message = error instanceof FolderReadError ? t(`folderImport.${error.reason}`) : (error as Error).message;
    toast.error(t("folderImport.failed", { name: name ?? "" }), { description: message });
  }, [t]);

  const run = useCallback(async (sources: (string | FolderSource)[]) => {
    if (working.current || !sources.length) return;
    if (sources.length > MAX_FOLDERS) { toast.error(t("folderImport.tooManyFolders")); return; }
    working.current = true; setBusy(true);
    try {
      for (const source of sources) {
        try {
          const copied = typeof source !== "string";
          const existing = queryClient.getQueryData<WorkspaceSummary[]>(["workspaces"]) ?? [];
          const workspace = typeof source === "string"
            ? await runtimeApi.openDirectory(workspaceRequest(source, existing, false))
            : await runtimeApi.importWorkspace(await source.read());
          openWorkspace(workspace);
          void queryClient.invalidateQueries({ queryKey: ["workspaces"] });
          if (copied) toast.success(t("folderImport.copied", { name: workspace.name }));
        } catch (error) { report(error, typeof source === "string" ? source.split(/[\\/]/).pop() : source.name); }
      }
    } finally { working.current = false; setBusy(false); }
  }, [openWorkspace, queryClient, report, t]);

  useEffect(() => onFileDrop((paths, point) => {
  ...
  const events = {
    ...
    onDrop: (event: DragEvent<HTMLElement>) => {
      ...
      if (event.target instanceof Element && event.target.closest("input, textarea, [role='menu']")) return;
      try { void run(droppedFolders(event.dataTransfer)); } catch (error) { report(error); }
    },
  };
