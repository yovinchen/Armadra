...
# Generated by AI Coding Canvas. Edits are overwritten (plan \u{a7}15.3).
set -g prefix None
set -g status off
...
impl TmuxBackend {
    ...
    /// The `@aicc-runtime` server option, if a server is running and stamped.
    async fn server_owner(&self) -> Option<String> {
        self.run(["show-options", "-s", "-v", "-q", "@aicc-runtime"])
            .await
            .ok()
    ...
    async fn stamp_server(&self) {
        if self.server_owner().await.is_none() {
            let _ = self
                .run(["set-option", "-s", "@aicc-runtime", &runtime_fingerprint()])
                .await;
        }
    }
...
impl TerminalBackend for TmuxBackend {
    ...
    async fn paste(&self, key: &SessionKey, text: &str, press_enter: bool) -> AppResult<()> {
        let session = self.session(key).await?;
        self.leave_copy_mode(&session).await;
        let buffer = format!("aicc-{}", uuid::Uuid::now_v7().simple());
    ...
    /// Destroys any `aicc-*` session the caller does not know about.
