...
async fn integration_routes_require_the_creating_workspace_and_explicit_continue() {
    ...
    let target=branches["branches"].as_array().unwrap().iter().find(|branch|branch["name"]=="feature/merge-target").unwrap()["oid"].clone();
    ...
    sqlx::query("UPDATE workspaces SET root_path=? WHERE id=?").bind(format!("{}/.",repo.display())).bind(&alias).execute(&pool).await.unwrap();
    let alias_base=format!("/api/workspaces/{alias}/git/repository");
    let (_,other)=request(&app,"GET",&format!("{alias_base}/integration"),Value::Null).await;
    ...
    assert_eq!(request(&app,"POST",&format!("{alias_base}/operations"),action.clone()).await.0,StatusCode::NOT_FOUND);
    let (status,operation)=request(&app,"POST",&format!("{base}/operations"),action).await;
    assert_eq!(status,StatusCode::OK,"{operation}");
    assert_eq!(wait(&app,&base,operation["id"].as_str().unwrap()).await["state"],"succeeded");
    assert_eq!(request(&app,"GET",&format!("{base}/integration"),Value::Null).await.1["kind"],"none");
    pool.close().await;
}
...
async fn cherry_pick_preview_and_empty_skip_remain_workspace_scoped() {
    ...
    git(&repo,&["add","picked.txt"]);git(&repo,&["commit","-m","pick fixture"]);git(&repo,&["switch","main"]);
    let base=format!("/api/workspaces/{}/git/repository",workspace.id);
    let (_,branches)=request(&app,"GET",&format!("{base}/branches"),Value::Null).await;
    let target=branches["branches"].as_array().unwrap().iter().find(|branch|branch["name"]=="feature/pick-target").unwrap()["oid"].as_str().unwrap().to_owned();
    let (status,preview)=request(&app,"GET",&format!("{base}/cherry-pick-preview?oid={target}"),Value::Null).await;
    assert_eq!(status,StatusCode::OK,"{preview}");assert_eq!(preview["subject"],"pick fixture");assert!(preview["patch"].as_str().unwrap().contains("picked content"));
    for expected_state in ["succeeded","awaitingResolution"] {
        ...
        assert_eq!(status,StatusCode::OK,"{operation}");
        assert_eq!(wait(&app,&base,operation["id"].as_str().unwrap()).await["state"],expected_state);
    }
    let (_,state)=request(&app,"GET",&format!("{base}/integration"),Value::Null).await;
    assert_eq!(state["empty"],true);assert_eq!(state["canSkip"],true);
    ...
    let alias_base=format!("/api/workspaces/{alias}/git/repository");
    let (_,other)=request(&app,"GET",&format!("{alias_base}/integration"),Value::Null).await;
    assert_eq!(other["owned"],false);assert_eq!(other["canSkip"],false);assert!(other["originalHead"].is_null());
    assert_eq!(request(&app,"POST",&format!("{alias_base}/operations"),action.clone()).await.0,StatusCode::NOT_FOUND);
    let (status,operation)=request(&app,"POST",&format!("{base}/operations"),action).await;
    assert_eq!(status,StatusCode::OK,"{operation}");assert_eq!(wait(&app,&base,operation["id"].as_str().unwrap()).await["state"],"succeeded");
    assert_eq!(std::fs::read_to_string(repo.join("picked.txt")).unwrap(),"picked content\n");
    assert_eq!(request(&app,"GET",&format!("{base}/integration"),Value::Null).await.1["head"],state["head"]);
    pool.close().await;
}
