...
func NewHandlerWithOptions(identity Identity, options Options) (http.Handler, error) {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if options.Identity != nil && (authMethod(r.URL.Path) || automationMethod(r.URL.Path) || githubMethod(r.URL.Path) || updatesMethod(r.URL.Path) || canvasMethod(r.URL.Path) || ownershipMethod(r.URL.Path) || settingsMethod(r.URL.Path) || filesystemMethod(r.URL.Path) || gitMethod(r.URL.Path) || sessionMethod(r.URL.Path) || agentMethod(r.URL.Path)) {
			...
			if agentMethod(r.URL.Path) {
				// A Host with no agent service authenticates first and then
				// answers UNSUPPORTED from inside; it never answers with an
				// empty status list, which a client would read as "no agent is
				// waiting for you" and stop showing a question somebody's CLI
				// is blocked on.
				agentRequest(w, r, identity, options.Identity, options.Agents)
				return
			}
...
func hello(w http.ResponseWriter, r *http.Request, identity Identity, surfaces helloSurfaces) {
	...
	// Advertised only when the agent surface is assembled. Like the others it
	// says the surface answers, not that this Host owns agent writes: a client
	// reads OwnershipService/List before it offers to answer an approval, and
	// answering one on the wrong side would leave a CLI blocked.
	if surfaces.agents {
		capabilities = append(capabilities, "agent.records.v1")
	}
