...
func buildExport(t *testing.T, root string) (string, *pb.MigrationExportManifest) {
 ...
 success BOOLEAN NOT NULL, checksum BLOB NOT NULL, execution_time BIGINT NOT NULL)`)
	names := []string{
		...
		"0004_agent_handoffs.sql", "0005_browser_sessions.sql", "0006_write_ownership.sql",
	}
	descriptions := []string{"initial", "agent mailbox", "retire kanban", "agent handoffs", "browser sessions", "write ownership"}
