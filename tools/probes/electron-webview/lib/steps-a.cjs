...
async function measureRound(d, zoom, viewport, label, winSize) {
  ...
  for (const id of BUTTONS) {
    ...
    hits.push({
      ...
      hitCorrectElement:
        Boolean(click) && click.id === id && click.elementFromPointId === id,
      deviationPx: click
  ...
  return {
    ...
    maxAbsDeviationPx: measured.length
      ? r2(
          Math.max(
            ...measured.flatMap((h) => [
              Math.abs(h.deviationPx.x),
              Math.abs(h.deviationPx.y),
            ]),
          ),
        )
      : null,
...
async function item1Zoom(d, zooms, winSize) {
  ...
  for (const zoom of zooms) {
    rounds.push(
      await measureRound(d, zoom, viewportFor(zoom), `zoom=${zoom}`, winSize),
    );
  }
  return {
...
async function item2Pan(d, zooms, winSize) {
  ...
  for (const zoom of zooms) {
    ...
    if (!from) {
      rounds.push({
        label: `pan zoom=${zoom}`,
        zoom,
        error: "no empty pane point for the drag",
      });
      continue;
    }
    ...
    await d.drag(
      from,
      { x: from.x + PAN.dx, y: from.y + PAN.dy },
      { steps: 15, settle: 250 },
    );
    const afterViewport = await d.canvas("getViewport()");
    const round = await measureRoundNoSetViewport(
      d,
      zoom,
      `pan zoom=${zoom}`,
      winSize,
  ...
  return {
    title:
      "same five targets after panning the canvas by (+300, -200), plus a fractional translate",
    rounds,
    pass: rounds.every(
      (r) => !r.error && r.allHitCorrectElement && r.skipped.length === 0,
    ),
  };
}
...
async function item3Raster(d, zooms, guestWebContents) {
  ...
  for (const zoom of zooms) {
    ...
    const image = await d.capture(
      crop,
      `raster-zoom-${String(zoom).replace(".", "_")}`,
    );
    samples.push({
      zoom,
      cropHostRect: {
      ...
      guestZoomFactor: guestWebContents
        ? guestWebContents.getZoomFactor()
        : null,
    ...
    one && two
      ? {
          intermediateFractionRatio_2_over_1: r4(
            two.metrics.intermediateFraction /
              Math.max(1e-6, one.metrics.intermediateFraction),
          ),
          maxGradientRatio_2_over_1: r4(
            two.metrics.maxHorizontalGradient /
              Math.max(1e-6, one.metrics.maxHorizontalGradient),
          ),
  ...
  return {
    title:
      "is guest text bitmap-scaled or re-rasterised at the composited scale?",
    samples,
    verdict,
...
async function item0InputRouting(d) {
  ...
  return {
    ...
    note: "webContents.sendInputEvent injects into the host RenderWidget and is not hit-tested into the guest OOPIF; CDP Input.* goes through the browser-process input router, which is the path a real mouse takes.",
    pass: true,
  };
}
