...
const modifierMask = (names) => names.reduce((acc, n) => acc | (MOD[n] || 0), 0);
...
function createDriver(win, outDir) {
  ...
  const guest = (id, code) =>
    host(`window.__canvas.guestEval(${JSON.stringify(id)}, ${JSON.stringify(code)})`);
  ...
  async function click(x, y, { button = 'left', modifiers = [] } = {}) {
    ...
    await move(px, py);
    await mouse('mousePressed', px, py, { button, clickCount: 1, buttons, modifiers: mods });
    await sleep(30);
    await mouse('mouseReleased', px, py, { button, clickCount: 1, buttons: 0, modifiers: mods });
    await sleep(90);
    return { x: px, y: py };
  }

  async function drag(from, to, { steps = 12, button = 'left', settle = 150 } = {}) {
  ...
  return { win, wc, cdp, host, canvas, guest, move, click, drag, wheel, key, capture, sleep };
}
