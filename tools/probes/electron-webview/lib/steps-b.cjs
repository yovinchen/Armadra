...
async function guestPoint(d, nodeId, id, fx = 0.5, fy = 0.5) {
  ...
  return {
    host: { x: rect.x + (box.x + box.w * fx) * scaleX, y: rect.y + (box.y + box.h * fy) * scaleY },
...
async function item4Interaction(d, guestFor) {
  ...
  const scrollBefore = await d.guest(NODE, 'document.getElementById("scroller").scrollTop');
  await d.wheel(scroller.host.x, scroller.host.y, 240);
  await d.wheel(scroller.host.x, scroller.host.y, 240);
  const scrollAfter = await d.guest(NODE, 'document.getElementById("scroller").scrollTop');
  ...
  for (const z of [0.5, 1, 2]) {
    ...
    const onMenu = (_event, params) => {
      mainEvent = { x: params.x, y: params.y, mediaType: params.mediaType, editFlags: Boolean(params.editFlags) };
    };
    ...
    contextMenu.push({
      ...
      mainEventVsGuestCssPx: mainEvent
        ? { x: r2(mainEvent.x - target.guest.x), y: r2(mainEvent.y - target.guest.y) }
        : null,
      mainEventVsHostWindowPx: mainEvent
        ? { x: r2(mainEvent.x - target.host.x), y: r2(mainEvent.y - target.host.y) }
        : null,
    });
  }
  ...
  const focused = await d.guest(NODE, 'document.activeElement && document.activeElement.id');
  for (const ch of ['a', 'b', 'c']) await d.key(ch);
  const afterTyping = await d.guest(NODE, 'document.getElementById("ime").value');
  ...
  const ime = {
    ...
    hostSyntheticTyping: { sent: 'abc', value: afterTyping, routed: afterTyping === 'abc' },
    guestSideComposition: imeComposition,
    events: imeEvents,
    limitation: 'composition events were dispatched inside the guest; no real OS IME was driven',
  };

  return {
    title: 'in-page scroll, <select>, context menu, text selection, keyboard/IME',
    scroll,
    select: selectResult,
    contextMenu,
    selection,
...
async function probeSelect(d) {
  ...
  const valueBefore = await d.guest(NODE, 'document.getElementById("sel").value');
  ...
  await d.sleep(300);
  const valueAfterKeys = await d.guest(NODE, 'document.getElementById("sel").value');
...
async function item5Wheel(d) {
  ...
  for (const spec of [
    ...
    { name: 'cmd + wheel over the guest', target: 'guest', modifiers: ['meta'] },
    { name: 'ctrl + wheel over the guest', target: 'guest', modifiers: ['control'] },
    { name: 'plain wheel over the empty pane (control case)', target: 'pane', modifiers: [] },
    { name: 'cmd + wheel over the empty pane (control case)', target: 'pane', modifiers: ['meta'] },
  ]) {
    await d.canvas(`setViewport(${v.x}, ${v.y}, ${zoom})`);
    await d.sleep(200);
    let point;
    ...
    const pointElement = await d.canvas(`elementAt(${Math.round(point.x)}, ${Math.round(point.y)})`);
    ...
    cases.push({
      ...
      canvasPanChanged: Math.abs(after.x - before.x) > 0.5 || Math.abs(after.y - before.y) > 0.5,
      pointElement,
  ...
  return {
    ...
    paneControlCaseWorks: overPane.some((c) => c.canvasZoomChanged || c.canvasPanChanged),
...
async function readGuests(d, ids) {
  ...
  for (const id of ids) {
    ...
    let error = null;
    try {
      loadCount = await d.guest(id, 'window.__probe && window.__probe.loadCount');
      persistentLoadCount = await d.guest(id, 'window.__probe && window.__probe.loadCountPersistent');
      clickCount = await d.guest(id, 'window.__probe && window.__probe.clickCount');
      formText = await d.guest(id, 'document.getElementById("ime").value');
      scrollTop = await d.guest(id, 'document.getElementById("scroller").scrollTop');
    } catch (err) {
...
async function item6Lifetime(d) {
  ...
  const record = async (name, extra = {}) => {
    ...
    steps.push({ step: name, guests, domOrder, nodeIds: await d.canvas('nodeIds()'), ...extra });
    return guests;
  };
  ...
  // is still there".
  for (const id of ids) {
    await d.guest(id, `document.getElementById("ime").value = ${JSON.stringify(`seed-${id}`)}; 1`);
  ...
  // (b) zoom
  await d.canvas(`setViewport(${viewportFor(0.25).x}, ${viewportFor(0.25).y}, 0.25)`);
  await d.sleep(300);
  ...
  if (from) {
    await d.drag(from, { x: from.x + 120, y: from.y + 90 }, { steps: 12, settle: 250 });
  }
  await d.canvas(`setViewport(${v.x}, ${v.y}, ${zoom})`);
    ...
    for (const id of ids) {
      ...
      if (reloaded) {
        reloads.push({
          ...
          persistentLoadCount: [prev.persistentLoadCount, now.persistentLoadCount],
  ...
  return {
    title: 'guest lifetime across drag / zoom / pan / array insert / delete / reorder',
    baseline,
    steps,
    reloads,
