...
const { item4Interaction, item5Wheel, item6Lifetime } = require('./lib/steps-b.cjs');
...
async function main() {
  ...
  await win.loadURL(`${base}/renderer/index.html?fixture=${encodeURIComponent(fixture)}`);

  const d = createDriver(win, OUT);
  await waitFor(() => d.host('Boolean(window.__canvas)'), 'renderer ready');
  await waitFor(
    () => d.host('window.__canvas.guestEval("wv-1", "Boolean(window.__probe)").catch(() => false)'),
    'guest wv-1 ready',
  );
  await waitFor(
    () => d.host('window.__canvas.guestEval("wv-2", "Boolean(window.__probe)").catch(() => false)'),
    'guest wv-2 ready',
  );
  await sleep(600);
  ...
  result.items['1_zoom_hit_testing'] = await run(() => item1Zoom(d, ZOOMS, WIN));
  ...
  result.items['4_in_page_interaction'] = await run(() => item4Interaction(d, guestFor));
  ...
  result.verdict = {
    ...
    perItem: Object.fromEntries(
      Object.entries(result.items).map(([k, v]) => [k, v.pass === true ? 'pass' : 'fail']),
    ),
  };
